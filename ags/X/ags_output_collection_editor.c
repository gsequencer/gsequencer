...
ags_output_collection_editor_init(AgsOutputCollectionEditor *output_collection_editor)
{
  ...
  output_collection_editor->pad = (GtkSpinButton *) gtk_spin_button_new_with_range(-1.0,
										   -1.0,
										   1.0);
  gtk_table_attach(GTK_TABLE(output_collection_editor),
		   GTK_WIDGET(output_collection_editor->pad),
		   1, 2,
		   0, 1,
		   GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND,
		   0, 0);
  ...
  output_collection_editor->soundcard_audio_channel = (GtkSpinButton *) gtk_spin_button_new_with_range(-1.0,
												       -1.0,
												       1.0);
  gtk_table_attach(GTK_TABLE(output_collection_editor),
		   GTK_WIDGET(output_collection_editor->soundcard_audio_channel),
		   1, 2,
		   3, 4,
		   GTK_FILL|GTK_EXPAND, GTK_FILL|GTK_EXPAND,
		   0, 0);
...
ags_output_collection_editor_apply(AgsApplicable *applicable)
{
  ...
  if(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(output_collection_editor->soundcard),
    ...
    pad = (guint) gtk_spin_button_get_value_as_int(output_collection_editor->pad);
    audio_channel = (guint) gtk_spin_button_get_value_as_int(output_collection_editor->audio_channel);

    soundcard_audio_channel = (guint) gtk_spin_button_get_value_as_int(output_collection_editor->soundcard_audio_channel);
...
ags_output_collection_editor_check(AgsOutputCollectionEditor *output_collection_editor)
{
  ...
  if(output_collection_editor->channel_type == AGS_TYPE_INPUT){
    gtk_spin_button_set_range(output_collection_editor->pad,
			      0.0,
  ...
  if(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(output_collection_editor->soundcard),
    ...
    gtk_spin_button_set_range(output_collection_editor->soundcard_audio_channel,
			      0.0,
