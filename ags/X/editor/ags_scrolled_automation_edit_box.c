...
{
  static GType ags_type_scrolled_automation_edit_box = 0;

  if(!ags_type_scrolled_automation_edit_box){
  ...
  return(ags_type_scrolled_automation_edit_box);
}
...
ags_scrolled_automation_edit_box_init(AgsScrolledAutomationEditBox *scrolled_automation_edit_box)
{
  ...
  /* viewport */
  scrolled_automation_edit_box->viewport = gtk_viewport_new(NULL,
							    NULL);
  g_object_set(scrolled_automation_edit_box->viewport,
	       "shadow-type", GTK_SHADOW_NONE,
	       NULL);
  gtk_container_add(scrolled_automation_edit_box,
		    scrolled_automation_edit_box->viewport);
