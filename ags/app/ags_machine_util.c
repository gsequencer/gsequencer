...
/**
 * ags_machine_util_new_quantum_synth:
 * 
 * Create #AgsQuantumSynth.
 * 
 * returns: the newly instantiated #AgsQuantumSynth
 * 
 * Since: 7.5.0
 */
GtkWidget*
ags_machine_util_new_quantum_synth()
{
  ...
  /* create quantum_synth */
  quantum_synth = ags_quantum_synth_new(G_OBJECT(default_soundcard));

  ags_window_add_machine(window,
			 AGS_MACHINE(quantum_synth));

  ags_connectable_connect(AGS_CONNECTABLE(quantum_synth));

  ags_audio_set_audio_channels(AGS_MACHINE(quantum_synth)->audio,
			       1, 0);
  
  ags_audio_set_pads(AGS_MACHINE(quantum_synth)->audio,
		     AGS_TYPE_INPUT,
		     128, 0);
  ags_audio_set_pads(AGS_MACHINE(quantum_synth)->audio,
		     AGS_TYPE_OUTPUT,
...
ags_machine_util_audio_unit_bridge_test_plugin(AgsAudioUnitPlugin *audio_unit_plugin)
{
  ...
  if(gsequencer_audio_unit_test_filename == NULL){
    ...
    argv[0] = g_strdup_printf("%s/Contents/MacOS/com.gsequencer.AudioUnitTest",
