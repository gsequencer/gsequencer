...
ags_notation_edit_init(AgsNotationEdit *notation_edit)
{
  ...
  notation_edit->cursor_position_x_256th = AGS_NOTATION_EDIT_DEFAULT_CURSOR_POSITION_X;
  ...
  event_controller =
    notation_edit->key_event_controller = gtk_event_controller_key_new();
  gtk_widget_add_controller((GtkWidget *) notation_edit->drawing_area,
			    event_controller);

  event_controller =
    notation_edit->gesture_click_event_controller = (GtkEventController *) gtk_gesture_click_new();
  gtk_widget_add_controller((GtkWidget *) notation_edit->drawing_area,
			    event_controller);

  event_controller =
    notation_edit->gesture_swipe_event_controller = (GtkEventController *) gtk_gesture_swipe_new();
  gtk_widget_add_controller((GtkWidget *) notation_edit->drawing_area,
			    event_controller);

  event_controller =
    notation_edit->motion_event_controller = (GtkEventController *) gtk_event_controller_motion_new();
  gtk_widget_add_controller((GtkWidget *) notation_edit->drawing_area,
			    event_controller);
...
ags_notation_edit_connect(AgsConnectable *connectable)
{
  ...
  /* event controller */
  g_signal_connect(notation_edit->key_event_controller, "key-pressed",
		   G_CALLBACK(ags_notation_edit_key_pressed_callback), notation_edit);
  
  g_signal_connect(notation_edit->key_event_controller, "key-released",
		   G_CALLBACK(ags_notation_edit_key_released_callback), notation_edit);

  g_signal_connect(notation_edit->key_event_controller, "modifiers",
		   G_CALLBACK(ags_notation_edit_modifiers_callback), notation_edit);

  g_signal_connect(notation_edit->gesture_click_event_controller, "pressed",
		   G_CALLBACK(ags_notation_edit_gesture_click_pressed_callback), notation_edit);

  g_signal_connect(notation_edit->gesture_click_event_controller, "released",
		   G_CALLBACK(ags_notation_edit_gesture_click_released_callback), notation_edit);

  g_signal_connect(notation_edit->motion_event_controller, "motion",
		   G_CALLBACK(ags_notation_edit_motion_callback), notation_edit);

  g_signal_connect(notation_edit->gesture_swipe_event_controller, "swipe",
...
ags_notation_edit_disconnect(AgsConnectable *connectable)
{
  ...
  /* event controller */
  g_object_disconnect(notation_edit->key_event_controller,
		      "any_signal::key-pressed",
		      G_CALLBACK(ags_notation_edit_key_pressed_callback),
		      notation_edit,
		      NULL);
  
  g_object_disconnect(notation_edit->key_event_controller,
		      "any_signal::key-released",
		      G_CALLBACK(ags_notation_edit_key_released_callback),
		      notation_edit,
		      NULL);

  g_object_disconnect(notation_edit->key_event_controller,
		      "any_signal::modifiers",
		      G_CALLBACK(ags_notation_edit_modifiers_callback),
		      notation_edit,
		      NULL);

  g_object_disconnect(notation_edit->gesture_click_event_controller,
		      "any_signal::pressed",
		      G_CALLBACK(ags_notation_edit_gesture_click_pressed_callback),
		      notation_edit,
		      NULL);

  g_object_disconnect(notation_edit->gesture_click_event_controller,
		      "any_signal::released",
		      G_CALLBACK(ags_notation_edit_gesture_click_released_callback),
		      notation_edit,
		      NULL);

  g_object_disconnect(notation_edit->motion_event_controller,
		      "any_signal::motion",
		      G_CALLBACK(ags_notation_edit_motion_callback),
		      notation_edit,
		      NULL);

  g_object_disconnect(notation_edit->gesture_swipe_event_controller,
		      "any_signal::swipe",
		      G_CALLBACK(ags_notation_edit_gesture_swipe_callback),
		      notation_edit,
		      NULL);
...
{  
  ...
  snap_to_zoom = gtk_check_button_get_active(composite_toolbar->snap_to_zoom);
  ...
  if(machine != NULL){
	...
	if((AGS_NOTATION_EDIT_KEY_L_SHIFT & (notation_edit->key_mask)) != 0 ||
	  ...
	  timestamp->timer.ags_offset.offset = AGS_NOTATION_DEFAULT_OFFSET * floor((double) notation_edit->cursor_position_x_256th / 16.0 / (double) AGS_NOTATION_DEFAULT_OFFSET);
	  ...
	  while((i = ags_notebook_next_active_tab(channel_selector,
		...
		note = ags_notation_find_point(AGS_NOTATION(list_notation->data),
					       (guint) floor(notation_edit->cursor_position_x_256th / 16.0), notation_edit->cursor_position_y,
					       FALSE);
		...
		note = ags_notation_find_exact_note_256th_point(AGS_NOTATION(list_notation->data),
								notation_edit->cursor_position_x_256th, notation_edit->cursor_position_y,
								FALSE);
		
		if(note != NULL){
		  if(note->x[1] - note->x[0] - 1 > 0){
		    ags_note_set_x1_256th(note,
					  note->x_256th[1] - 1);
		  }else{
		    ags_note_set_x1_256th(note,
					  note->x_256th[0] + 1);
	  ...
	  /* position cursor */
	  if(notation_edit->cursor_position_x_256th > 0){
	    if(snap_to_zoom){
	      if((gint) floor(notation_edit->cursor_position_x_256th) - (gint) floor(16.0 * zoom_factor) > 0){
		notation_edit->cursor_position_x_256th -= (gint) floor(16.0 * zoom_factor);
	      }else{
		notation_edit->cursor_position_x_256th = 0;
	      }
	    }else{
	      if((gint) floor(notation_edit->cursor_position_x_256th) - (1) > 0){
		notation_edit->cursor_position_x_256th -= (1);
	      }else{
		notation_edit->cursor_position_x_256th = 0;
	      }
	    }
	  ...
	  x0_offset = (gdouble) floor((double) notation_edit->cursor_position_x_256th / 16.0) * (gdouble) notation_edit->control_width;
	...
	if((AGS_NOTATION_EDIT_KEY_L_SHIFT & (notation_edit->key_mask)) != 0 ||
	  ...
	  timestamp->timer.ags_offset.offset = AGS_NOTATION_DEFAULT_OFFSET * (guint64) floor((double) notation_edit->cursor_position_x_256th / 16.0 / (double) AGS_NOTATION_DEFAULT_OFFSET);
	  ...
	  while((i = ags_notebook_next_active_tab(channel_selector,
	    ...
	    if(list_notation != NULL){
	      ...
	      if(snap_to_zoom){
		note = ags_notation_find_point(AGS_NOTATION(list_notation->data),
					       (gint) floor(notation_edit->cursor_position_x_256th / 16.0), notation_edit->cursor_position_y,
					       FALSE);
	  ...
	  /* position cursor */
	  if(snap_to_zoom){
	    if((gint) floor((double) notation_edit->cursor_position_x_256th / 16.0) < AGS_NAVIGATION_MAX_POSITION_TICS){
	      notation_edit->cursor_position_x_256th += (gint) floor(16.0 * zoom_factor);
	      
	      do_feedback = TRUE;
	    }
	  }else{
	    if(notation_edit->cursor_position_x_256th < AGS_NAVIGATION_MAX_POSITION_TICS){
	      notation_edit->cursor_position_x_256th += (1);
	...
	ags_note_set_x0_256th(note,
			      notation_edit->cursor_position_x_256th);
	...
	if(snap_to_zoom){
	  if(!pattern_mode){
	    ...
	    ags_note_set_x1_256th(note,
				  notation_edit->cursor_position_x_256th + 16);
	  }
	}else{
	  if(!pattern_mode){
	    ags_note_set_x1_256th(note,
				  notation_edit->cursor_position_x_256th + (gint) 1);
	  }else{
	    ags_note_set_x1_256th(note,
				  notation_edit->cursor_position_x_256th + 16);
	  }
							     ...
							     gdouble x, gdouble y)
{
  ...
  /* cursor position */
  notation_edit->cursor_position_x_256th = (guint) (zoom_factor * (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)))) * 16.0 / notation_edit->control_width;
  notation_edit->cursor_position_x_256th = (zoom_factor * 16.0) * floor(notation_edit->cursor_position_x_256th / (zoom_factor * 16.0));
...
#ifdef AGS_DEBUG
  g_message("%lu %lu", notation_edit->cursor_position_x_256th, notation_edit->cursor_position_y);
#endif
}
						      ...
						      gdouble x, gdouble y)
{
  ...
  if(!pattern_mode){
	...
	note->x[1] = new_x;
      ...
      new_x_256th = (gint) (zoom_factor * (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)))) / ((double) notation_edit->control_width / 16.0);
							...
							gdouble x, gdouble y)
{
  ...
  /* cursor position */
  notation_edit->resize_position_x = (gint) (zoom_factor * (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)))) / notation_edit->control_width;
  notation_edit->resize_position_x = zoom_factor * floor(notation_edit->resize_position_x / zoom_factor);

  notation_edit->resize_position_y = (gint) ((y + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->vscrollbar))) / notation_edit->control_height);
}
							 ...
							 gdouble x, gdouble y)
{
  if(x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)) >= 0.0){
    notation_edit->selection_x1 = (gint) (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)));
  }else{
    notation_edit->selection_x1 = 0.0;
  }
    
  if(y + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->vscrollbar)) >= 0.0){
    notation_edit->selection_y1 = (gint) y + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->vscrollbar));
  }else{
							 ...
							 gdouble x, gdouble y)
{
  ...
  note_x0 = (gint) notation_edit->resize_position_x;
  note_y = (gint) notation_edit->resize_position_y;

  current_note_x1 = (gint) (zoom_factor * (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)))) / notation_edit->control_width;
							    ...
							    gdouble x, gdouble y)
{
  ...
  /* cursor position */
  notation_edit->cursor_position_x_256th = (gint) (zoom_factor * (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)))) * 16.0 / notation_edit->control_width;
  notation_edit->cursor_position_x_256th = (zoom_factor * 16.0) * floor(notation_edit->cursor_position_x_256th / (zoom_factor * 16.0));
  
  notation_edit->cursor_position_y = (gint) ((y + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->vscrollbar))) / notation_edit->control_height);
}
...
ags_notation_edit_drawing_area_button_press_select_note(GtkWidget *editor,
							...
							gdouble x, gdouble y)
{
  notation_edit->selection_x0 = (gint) x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar));
  notation_edit->selection_x1 = notation_edit->selection_x0;
    
  notation_edit->selection_y0 = (gint) y + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->vscrollbar));
...
{
  double zoom_factor;
  gint x0_256th, x1_256th, y0, y1;

  /* zoom */
  zoom_factor = exp2(6.0 - (double) gtk_combo_box_get_active((GtkComboBox *) AGS_COMPOSITE_TOOLBAR(toolbar)->zoom));

  /* region */
  x0_256th = (gint) ((zoom_factor * notation_edit->selection_x0) / notation_edit->control_width * 16.0);

  y0 = (gint) (notation_edit->selection_y0 / notation_edit->control_height);
    
  x1_256th = (gint) ((zoom_factor * (x + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)))) / notation_edit->control_width * 16.0);
    
  y1 = (gint) ((y + gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->vscrollbar))) / notation_edit->control_height);

  /* select region */
  ags_composite_editor_select_region((AgsCompositeEditor *) editor,
				     ((x0_256th >= 0) ? x0_256th: 0), (gdouble) ((y0 >= 0) ? y0: 0),
				     ((x1_256th >= 0) ? x1_256th: 0), (gdouble) ((y1 >= 0) ? y1: 0));
}
...
ags_notation_edit_draw_segment(AgsNotationEdit *notation_edit, cairo_t *cr)
{
  ...
  if(width_fits){
    ...
    x0 = notation_edit->control_width - ((gint) gtk_adjustment_get_value(hscrollbar_adjustment) % notation_edit->control_width);
  }

  if(height_fits){
    y0 = 0;
  }else{
    y0 = notation_edit->control_height - ((gint) gtk_adjustment_get_value(vscrollbar_adjustment) % notation_edit->control_height);
  }
...
ags_notation_edit_draw_cursor(AgsNotationEdit *notation_edit, cairo_t *cr)
{
  ...
  /* get offset */
  x = (((double) notation_edit->cursor_position_x_256th / 16.0) * (double) notation_edit->control_width) - (gtk_adjustment_get_value(gtk_scrollbar_get_adjustment(notation_edit->hscrollbar)) * zoom_factor);
