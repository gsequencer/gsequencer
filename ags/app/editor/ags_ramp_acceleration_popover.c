...
ags_ramp_acceleration_popover_init(AgsRampAccelerationPopover *ramp_acceleration_popover)
{
  ...
  gtk_widget_set_halign(ramp_acceleration_popover->action_area,
			GTK_ALIGN_END);
  ...
  gtk_popover_set_default_widget(ramp_acceleration_popover,
				 ramp_acceleration_popover->activate_button);
}
...
ags_ramp_acceleration_popover_reset(AgsApplicable *applicable)
{
  ...
  if(position != -1){
    gtk_combo_box_set_active(ramp_acceleration_popover->port,
			     position);
  }
						    ...
						    AgsRampAccelerationPopover *ramp_acceleration_popover)
{
  ...
  if(keyval == GDK_KEY_Tab ||
    ...
    case GDK_KEY_Escape:
      {
	gtk_popover_popdown(ramp_acceleration_popover);	
      }
      break;
    }
  }
