...
ags_select_note_popover_get_type(void)
{
  ...
  if(g_once_init_enter (&g_define_type_id__volatile)){
    ...
    g_once_init_leave(&g_define_type_id__volatile, ags_type_select_note_popover);
  }
...
ags_select_note_popover_init(AgsSelectNotePopover *select_note_popover)
{
  ...
  gtk_widget_set_halign(select_note_popover->action_area,
			GTK_ALIGN_END);
  ...
  gtk_popover_set_default_widget(select_note_popover,
				 select_note_popover->activate_button);
}
					      ...
					      AgsSelectNotePopover *select_note_popover)
{
  ...
  if(keyval == GDK_KEY_Tab ||
    ...
    case GDK_KEY_Escape:
      {
	gtk_popover_popdown(select_note_popover);	
      }
      break;
    }
  }
