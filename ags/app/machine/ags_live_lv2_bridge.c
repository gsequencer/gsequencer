...
static void ags_live_lv2_bridge_class_init(AgsLiveLv2BridgeClass *live_lv2_bridge);
static void ags_live_lv2_bridge_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_live_lv2_bridge_init(AgsLiveLv2Bridge *live_lv2_bridge);
static void ags_live_lv2_bridge_set_property(GObject *gobject,
					     guint prop_id,
					     const GValue *value,
					     GParamSpec *param_spec);
static void ags_live_lv2_bridge_get_property(GObject *gobject,
					     guint prop_id,
					     GValue *value,
					     GParamSpec *param_spec);
static void ags_live_lv2_bridge_finalize(GObject *gobject);

static void ags_live_lv2_bridge_connect(AgsConnectable *connectable);
static void ags_live_lv2_bridge_disconnect(AgsConnectable *connectable);

static void ags_live_lv2_bridge_resize_audio_channels(AgsMachine *machine,
						      guint audio_channels, guint audio_channels_old,
						      gpointer data);
static void ags_live_lv2_bridge_resize_pads(AgsMachine *machine, GType channel_type,
					    guint pads, guint pads_old,
					    gpointer data);

static void ags_live_lv2_bridge_map_recall(AgsMachine *machine);
...
ags_live_lv2_bridge_get_type(void)
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_live_lv2_bridge);
  }
...
ags_live_lv2_bridge_init(AgsLiveLv2Bridge *live_lv2_bridge)
{
  ...
  AGS_EFFECT_BRIDGE(AGS_MACHINE(live_lv2_bridge)->bridge)->bulk_input = (GtkWidget *) ags_effect_bulk_new(audio,
													  AGS_TYPE_INPUT);
  ags_effect_bulk_set_flags(AGS_EFFECT_BULK(AGS_EFFECT_BRIDGE(AGS_MACHINE(live_lv2_bridge)->bridge)->bulk_input),
...
ags_live_lv2_bridge_disconnect(AgsConnectable *connectable)
{
  ...
  ags_live_lv2_bridge_parent_connectable_interface->disconnect(connectable);

  live_lv2_bridge = AGS_LIVE_LV2_BRIDGE(connectable);

  /* program */
  if(live_lv2_bridge->program != NULL){
    g_object_disconnect(G_OBJECT(live_lv2_bridge->program), "any_signal::changed",
			G_CALLBACK(ags_live_lv2_bridge_program_changed_callback),
			live_lv2_bridge,
			NULL);
  }
  ...
  while(list != NULL){
    ...
    if(bulk_member->widget_type == AGS_TYPE_DIAL){
      g_object_disconnect(GTK_WIDGET(control),
			  ...
			  G_CALLBACK(ags_live_lv2_bridge_dial_changed_callback),
			  live_lv2_bridge,
			  NULL);
    }else if(bulk_member->widget_type == GTK_TYPE_SCALE){
      g_object_disconnect(GTK_WIDGET(control),
			  "any_signal::value-changed",
			  G_CALLBACK(ags_live_lv2_bridge_scale_changed_callback),
			  live_lv2_bridge,
			  NULL);
    }else if(bulk_member->widget_type == GTK_TYPE_SPIN_BUTTON){
      g_object_disconnect(GTK_WIDGET(control),
			  "any_signal::value-changed",
			  G_CALLBACK(ags_live_lv2_bridge_spin_button_changed_callback),
			  live_lv2_bridge,
			  NULL);
    }else if(bulk_member->widget_type == GTK_TYPE_CHECK_BUTTON){
      g_object_disconnect(GTK_WIDGET(control),
			  "any_signal::clicked",
			  G_CALLBACK(ags_live_lv2_bridge_check_button_clicked_callback),
			  live_lv2_bridge,
			  NULL);
    }else if(bulk_member->widget_type == GTK_TYPE_TOGGLE_BUTTON){
      g_object_disconnect(GTK_WIDGET(control),
			  "any_signal::clicked",
			  G_CALLBACK(ags_live_lv2_bridge_toggle_button_clicked_callback),
			  live_lv2_bridge,
			  NULL);
    }else if(bulk_member->widget_type == GTK_TYPE_BUTTON){
      g_object_disconnect(GTK_WIDGET(control),
			  "any_signal::clicked",
			  G_CALLBACK(ags_live_lv2_bridge_button_clicked_callback),
			  live_lv2_bridge,
			  NULL);
    }
