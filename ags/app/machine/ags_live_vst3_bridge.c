...
static void ags_live_vst3_bridge_class_init(AgsLiveVst3BridgeClass *live_vst3_bridge);
static void ags_live_vst3_bridge_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_live_vst3_bridge_init(AgsLiveVst3Bridge *live_vst3_bridge);
static void ags_live_vst3_bridge_set_property(GObject *gobject,
					      guint prop_id,
					      const GValue *value,
					      GParamSpec *param_spec);
static void ags_live_vst3_bridge_get_property(GObject *gobject,
					      guint prop_id,
					      GValue *value,
					      GParamSpec *param_spec);
static void ags_live_vst3_bridge_finalize(GObject *gobject);

static void ags_live_vst3_bridge_connect(AgsConnectable *connectable);
static void ags_live_vst3_bridge_disconnect(AgsConnectable *connectable);

static void ags_live_vst3_bridge_resize_audio_channels(AgsMachine *machine,
						       guint audio_channels, guint audio_channels_old,
						       gpointer data);
static void ags_live_vst3_bridge_resize_pads(AgsMachine *machine, GType type,
					     guint pads, guint pads_old,
					     gpointer data);

static void ags_live_vst3_bridge_map_recall(AgsMachine *machine);
...
ags_live_vst3_bridge_init(AgsLiveVst3Bridge *live_vst3_bridge)
{
  ...
  AGS_EFFECT_BULK(AGS_EFFECT_BRIDGE(AGS_MACHINE(live_vst3_bridge)->bridge)->bulk_input)->parent_bridge = (GtkWidget *) AGS_MACHINE(live_vst3_bridge)->bridge;
