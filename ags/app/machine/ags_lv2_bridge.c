...
static void ags_lv2_bridge_class_init(AgsLv2BridgeClass *lv2_bridge);
static void ags_lv2_bridge_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_lv2_bridge_init(AgsLv2Bridge *lv2_bridge);
static void ags_lv2_bridge_set_property(GObject *gobject,
					guint prop_id,
					const GValue *value,
					GParamSpec *param_spec);
static void ags_lv2_bridge_get_property(GObject *gobject,
					guint prop_id,
					GValue *value,
					GParamSpec *param_spec);
static void ags_lv2_bridge_finalize(GObject *gobject);

static void ags_lv2_bridge_connect(AgsConnectable *connectable);
static void ags_lv2_bridge_disconnect(AgsConnectable *connectable);

static void ags_lv2_bridge_resize_audio_channels(AgsMachine *machine,
						 guint audio_channels, guint audio_channels_old,
						 gpointer data);
static void ags_lv2_bridge_resize_pads(AgsMachine *machine, GType channel_type,
				       guint pads, guint pads_old,
				       gpointer data);

static void ags_lv2_bridge_map_recall(AgsMachine *machine);
 ...
 * @title: AgsLv2Bridge
 * @section_id:
 * @include: ags/app/ags_lv2_bridge.h
 *
...
ags_lv2_bridge_get_type(void)
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_lv2_bridge);
  }
...
ags_lv2_bridge_init(AgsLv2Bridge *lv2_bridge)
{
  ...
  gtk_widget_set_margin_top((GtkWidget *) lv2_bridge->vbox,
			    AGS_UI_PROVIDER_DEFAULT_MARGIN);
  gtk_widget_set_margin_bottom((GtkWidget *) lv2_bridge->vbox,
			       AGS_UI_PROVIDER_DEFAULT_MARGIN);

  gtk_widget_set_margin_start((GtkWidget *) lv2_bridge->vbox,
			      AGS_UI_PROVIDER_DEFAULT_MARGIN);
  gtk_widget_set_margin_end((GtkWidget *) lv2_bridge->vbox,
			    AGS_UI_PROVIDER_DEFAULT_MARGIN);
  ...
  AGS_EFFECT_BRIDGE(AGS_MACHINE(lv2_bridge)->bridge)->bulk_input = (GtkWidget *) ags_effect_bulk_new(audio,
												     AGS_TYPE_INPUT);
  ags_effect_bulk_set_flags(AGS_EFFECT_BULK(AGS_EFFECT_BRIDGE(AGS_MACHINE(lv2_bridge)->bridge)->bulk_input),
			    (AGS_EFFECT_BULK_HIDE_BUTTONS |
			     AGS_EFFECT_BULK_HIDE_ENTRIES |
			     AGS_EFFECT_BULK_SHOW_LABELS));

  gtk_widget_set_valign((GtkWidget *) AGS_EFFECT_BRIDGE(AGS_MACHINE(lv2_bridge)->bridge)->bulk_input,
			GTK_ALIGN_FILL);
  gtk_widget_set_halign((GtkWidget *) AGS_EFFECT_BRIDGE(AGS_MACHINE(lv2_bridge)->bridge)->bulk_input,
			GTK_ALIGN_FILL);
  
  AGS_EFFECT_BULK(AGS_EFFECT_BRIDGE(AGS_MACHINE(lv2_bridge)->bridge)->bulk_input)->parent_bridge = (GtkWidget *) AGS_MACHINE(lv2_bridge)->bridge;
...
ags_lv2_bridge_connect(AgsConnectable *connectable)
{
  ...
  while(list != NULL){
    ...
    if(bulk_member->widget_type == AGS_TYPE_DIAL){
      ...
      g_signal_connect_after(GTK_WIDGET(control), "value-changed",
			     G_CALLBACK(ags_lv2_bridge_scale_changed_callback), lv2_bridge);
...
ags_lv2_bridge_load_preset(AgsLv2Bridge *lv2_bridge)
{
  ...
  while(list != NULL){
	...
	success = TRUE;
      ...
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(lv2_bridge->preset),
