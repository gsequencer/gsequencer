...
static void ags_raven_synth_class_init(AgsRavenSynthClass *raven_synth);
static void ags_raven_synth_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_raven_synth_init(AgsRavenSynth *raven_synth);
static void ags_raven_synth_finalize(GObject *gobject);

static void ags_raven_synth_connect(AgsConnectable *connectable);
static void ags_raven_synth_disconnect(AgsConnectable *connectable);

static void ags_raven_synth_show(GtkWidget *widget);

static void ags_raven_synth_resize_audio_channels(AgsMachine *machine,
						  guint audio_channels, guint audio_channels_old,
						  gpointer data);
static void ags_raven_synth_resize_pads(AgsMachine *machine, GType channel_type,
					guint pads, guint pads_old,
					gpointer data);

static void ags_raven_synth_map_recall(AgsMachine *machine);

static void ags_raven_synth_input_map_recall(AgsRavenSynth *raven_synth,
					     guint audio_channel_start,
					     guint input_pad_start);
static void ags_raven_synth_output_map_recall(AgsRavenSynth *raven_synth,
					      guint audio_channel_start,
					      guint output_pad_start);

static void ags_raven_synth_refresh_port(AgsMachine *machine);
