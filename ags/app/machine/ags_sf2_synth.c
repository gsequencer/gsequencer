...
static void ags_sf2_synth_class_init(AgsSF2SynthClass *sf2_synth);
static void ags_sf2_synth_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_sf2_synth_init(AgsSF2Synth *sf2_synth);
static void ags_sf2_synth_finalize(GObject *gobject);

static void ags_sf2_synth_connect(AgsConnectable *connectable);
static void ags_sf2_synth_disconnect(AgsConnectable *connectable);

static void ags_sf2_synth_show(GtkWidget *widget);

static void ags_sf2_synth_wah_wah_draw_function(GtkDrawingArea *area,
						cairo_t *cr,
						int width,
						int height,
						AgsSF2Synth *sf2_synth);

static void ags_sf2_synth_resize_audio_channels(AgsMachine *machine,
						guint audio_channels, guint audio_channels_old,
						gpointer data);
static void ags_sf2_synth_resize_pads(AgsMachine *machine, GType channel_type,
				      guint pads, guint pads_old,
				      gpointer data);

static void ags_sf2_synth_map_recall(AgsMachine *machine);

static void ags_sf2_synth_input_map_recall(AgsSF2Synth *sf2_synth,
					   guint audio_channel_start,
					   guint input_pad_start);
static void ags_sf2_synth_output_map_recall(AgsSF2Synth *sf2_synth,
					    guint audio_channel_start,
					    guint output_pad_start);

static gint ags_sf2_synth_int_compare_func(gconstpointer a,
					   gconstpointer b);

static void ags_sf2_synth_refresh_port(AgsMachine *machine);
...
//			       12);
  ...
  gtk_widget_set_vexpand((GtkWidget *) chorus_grid,
			 FALSE);  
  ...
  sf2_synth->low_pass_enabled = (GtkCheckButton *) gtk_check_button_new_with_label(i18n("enabled"));
  gtk_grid_attach(low_pass_grid,
		  (GtkWidget *) sf2_synth->low_pass_enabled,
		  0, 0,
		  2, 1);
  ...
  sf2_synth->low_pass_cut_off_frequency = (AgsDial *) ags_dial_new();

  ags_dial_set_radius(sf2_synth->low_pass_cut_off_frequency,
		      12);

  adjustment = ags_dial_get_adjustment(sf2_synth->low_pass_cut_off_frequency);
  ...
  gtk_grid_attach(low_pass_grid,
		  (GtkWidget *) sf2_synth->low_pass_cut_off_frequency,
		  1, 1,
		  1, 1);
  ...
  sf2_synth->low_pass_filter_gain = (AgsDial *) ags_dial_new();

  ags_dial_set_radius(sf2_synth->low_pass_filter_gain,
		      12);

  adjustment = ags_dial_get_adjustment(sf2_synth->low_pass_filter_gain);
  ...
  gtk_grid_attach(low_pass_grid,
		  (GtkWidget *) sf2_synth->low_pass_filter_gain,
		  1, 2,
		  1, 1);
  ...
  sf2_synth->low_pass_no_clip = (AgsDial *) ags_dial_new();

  ags_dial_set_radius(sf2_synth->low_pass_no_clip,
		      12);

  adjustment = ags_dial_get_adjustment(sf2_synth->low_pass_no_clip);
  ...
  gtk_grid_attach(low_pass_grid,
		  (GtkWidget *) sf2_synth->low_pass_no_clip,
		  1, 3,
		  1, 1);
...
//			 G_CALLBACK(ags_sf2_synth_chorus_enabled_callback), sf2_synth);
  ...
  /* low-pass */
  g_signal_connect_after(sf2_synth->low_pass_enabled, "toggled",
			 G_CALLBACK(ags_sf2_synth_low_pass_enabled_callback), sf2_synth);

  g_signal_connect_after(sf2_synth->low_pass_cut_off_frequency, "value-changed",
			 G_CALLBACK(ags_sf2_synth_low_pass_cut_off_frequency_callback), sf2_synth);

  g_signal_connect_after(sf2_synth->low_pass_filter_gain, "value-changed",
			 G_CALLBACK(ags_sf2_synth_low_pass_filter_gain_callback), sf2_synth);

  g_signal_connect_after(sf2_synth->low_pass_no_clip, "value-changed",
			 G_CALLBACK(ags_sf2_synth_low_pass_no_clip_callback), sf2_synth);
...
ags_sf2_synth_disconnect(AgsConnectable *connectable)
{
  ...
  /* low-pass */
  g_object_disconnect((GObject *) sf2_synth->low_pass_enabled,
		      "any_signal::toggled",
		      G_CALLBACK(ags_sf2_synth_low_pass_enabled_callback),
		      sf2_synth,
		      NULL);

  g_object_disconnect(sf2_synth->low_pass_cut_off_frequency,
		      "any_signal::value-changed",
		      G_CALLBACK(ags_sf2_synth_low_pass_cut_off_frequency_callback),
		      sf2_synth,
		      NULL);

  g_object_disconnect(sf2_synth->low_pass_filter_gain,
		      "any_signal::value-changed",
		      G_CALLBACK(ags_sf2_synth_low_pass_filter_gain_callback),
		      sf2_synth,
		      NULL);

  g_object_disconnect(sf2_synth->low_pass_no_clip,
		      "any_signal::value-changed",
		      G_CALLBACK(ags_sf2_synth_low_pass_no_clip_callback),
		      sf2_synth,
		      NULL);
