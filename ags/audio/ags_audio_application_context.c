...
ags_audio_application_context_connect(AgsConnectable *connectable)
{
  ...
  /* sequencer */
  sequencer = 
    start_sequencer = ags_sound_provider_get_sequencer(AGS_SOUND_PROVIDER(audio_application_context));
...
GList*
ags_audio_application_context_get_program(AgsSoundProvider *sound_provider)
{
  AgsApplicationContext *application_context;
  ...
  program = g_list_copy_deep(AGS_AUDIO_APPLICATION_CONTEXT(application_context)->program,
			     (GCopyFunc) g_object_ref,
			     NULL);
...
void
ags_audio_application_context_set_program(AgsSoundProvider *sound_provider,
					  GList *program)
{
  AgsApplicationContext *application_context;
  ...
  if(AGS_AUDIO_APPLICATION_CONTEXT(application_context)->program == program){
    g_rec_mutex_unlock(application_context_mutex);

    return;
  }

  g_list_free_full(AGS_AUDIO_APPLICATION_CONTEXT(application_context)->program,
		   (GDestroyNotify) g_object_unref);
...
ags_audio_application_context_prepare(AgsApplicationContext *application_context)
{
  ...
  g_thread_new("Advanced Gtk+ Sequencer - server main loop",
	       (GThreadFunc) ags_audio_application_context_server_main_loop_thread,
	       main_loop);
  ...
  g_thread_new("Advanced Gtk+ Sequencer - audio main loop",
	       (GThreadFunc) ags_audio_application_context_audio_main_loop_thread,
	       main_loop);
#else
  audio_application_context->audio_main_context = NULL;  
#endif
