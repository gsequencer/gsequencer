...
ags_channel_get_type (void)
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_channel);
  }
 ...
 */
gboolean
ags_channel_test_flags(AgsChannel *channel, AgsChannelFlags flags)
{
  gboolean retval;  
 ...
 */
void
ags_channel_set_flags(AgsChannel *channel, AgsChannelFlags flags)
{
  GRecMutex *channel_mutex;
 ...
 */
void
ags_channel_unset_flags(AgsChannel *channel, AgsChannelFlags flags)
{  
  GRecMutex *channel_mutex;
 ...
 */
gboolean
ags_channel_test_ability_flags(AgsChannel *channel, AgsSoundAbilityFlags ability_flags)
{
  gboolean retval;  
 ...
 */
void
ags_channel_set_ability_flags(AgsChannel *channel, AgsSoundAbilityFlags ability_flags)
{
  AgsAudio *audio;
  ...
  AgsSoundAbilityFlags channel_ability_flags;
 ...
 */
void
ags_channel_unset_ability_flags(AgsChannel *channel, AgsSoundAbilityFlags ability_flags)
{
  AgsPlaybackDomain *playback_domain;
  AgsPlayback *playback;
 ...
 */
gboolean
ags_channel_test_behaviour_flags(AgsChannel *channel, AgsSoundBehaviourFlags behaviour_flags)
{
  gboolean retval;  
 ...
 */
void
ags_channel_set_behaviour_flags(AgsChannel *channel, AgsSoundBehaviourFlags behaviour_flags)
{
  GRecMutex *channel_mutex;
 ...
 */
void
ags_channel_unset_behaviour_flags(AgsChannel *channel, AgsSoundBehaviourFlags behaviour_flags)
{
  GRecMutex *channel_mutex;
...
gboolean
ags_channel_check_loop(AgsChannel *output,
		       AgsAudio *audio,
		       guint level)
{
  ...
  audio_matches = (audio == current_audio) ? TRUE: FALSE;
  ...
  if(!audio_matches){
    ...
    if(ags_audio_test_flags(current_audio, AGS_AUDIO_ASYNC)){
      current_output = ags_channel_nth(current_start_output,
				       current_audio_channel);

      while(current_output != NULL){
	AgsChannel *next_pad;

	audio_matches = ags_channel_check_loop(current_output,
					       audio,
					       level + 1);
	...
	/* iterate */
	next_pad = ags_channel_next_pad(current_output);

	g_object_unref(current_output);

	current_output = next_pad;
      }
    ...
    if(current_start_output != NULL){
      g_object_unref(current_start_output);
    }
  }
		     ...
		     GError **error)
{
  ...
  if(channel != NULL &&
    ...
    g_object_ref(start_iter);

    g_object_get(start_iter,
		 "audio", &audio,
		 NULL);

    level = 0;
    
    audio_matches = ags_channel_check_loop(start_iter,
					   audio,
					   level);
  ...
  if(AGS_IS_INPUT(channel)){
    if(link == NULL){
      ...
      if(ags_audio_test_flags(audio, AGS_AUDIO_INPUT_HAS_RECYCLING)){
	first_recycling =
	  last_recycling = g_object_new(AGS_TYPE_RECYCLING,
					"channel", channel,
					"output-soundcard", output_soundcard,
					"output-soundcard-channel", output_soundcard_channel,
					"input-soundcard", input_soundcard,
  ...
  if(old_channel_link != NULL){
    if(AGS_IS_OUTPUT(old_channel_link)){
      AgsRecycling *first_recycling, *last_recycling;
  ...
  if(old_link_link != NULL){
    if(AGS_IS_OUTPUT(old_link_link)){
      AgsRecycling *first_recycling, *last_recycling;
			    ...
			    AgsRecycling *first_recycling, AgsRecycling *last_recycling)
{
  ...
  /* link recycling */
  if(AGS_IS_INPUT(channel) &&
     (AGS_AUDIO_ASYNC & (audio_flags)) != 0){
    AgsChannel *prev, *next;
    AgsRecycling *prev_last_recycling, *next_first_recycling;
    
    if(first_recycling != NULL){
      prev = ags_channel_prev_with_recycling(channel);
      next = ags_channel_next_with_recycling(channel);
	...
	g_object_unref(next);
      ...
      if(prev_last_recycling != NULL){
	g_object_unref(prev_last_recycling);
      }

      if(next_first_recycling != NULL){
	g_object_unref(next_first_recycling);
      }
    }
  }
  ...
  if(level_channel != NULL){
    ...
    if(level_audio != NULL){
      g_object_unref(level_audio);
    }
    
    g_object_unref(level_channel);

    if(level_link != NULL){
      g_object_unref(level_link);
    }
  }else{
    if(first_recycling == NULL){
      /* reset recycling context */
  ...
  if(!is_output){
	...
	nth_channel = ags_channel_nth(output, audio_channel);
	...
	nth_channel = ags_channel_nth(output, line);
...
void
ags_channel_real_set_format(AgsChannel *channel, AgsSoundcardFormat format)
{
  AgsChannel *link;
  AgsRecycling *recycling;
 ...
 */
void
ags_channel_set_format(AgsChannel *channel, AgsSoundcardFormat format)
{
  if(!AGS_IS_CHANNEL(channel)){
    return;
  }
