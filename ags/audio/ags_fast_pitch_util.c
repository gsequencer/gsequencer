...
ags_fast_pitch_util_pitch_s8(AgsFastPitchUtil *fast_pitch_util)
{
  ...
  for(i = 0, j = 0; i < buffer_length; i++, j++){
    ...
    new_freq = (exp2((fast_pitch_util->base_key + (fast_pitch_util->tuning / 100.0)) / 12.0) * 440.0) * (sin((vibrato_lfo_offset + i) * 2.0 * M_PI * vibrato_lfo_freq * exp2(vibrato_tuning / 1200.0) / samplerate) * vibrato_lfo_depth);
