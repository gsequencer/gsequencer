...
ags_sf2_synth_util_copy(AgsSF2SynthUtil *ptr)
{
  ...
  new_ptr->sample_buffer = NULL;
  new_ptr->im_buffer = NULL;

  if(ptr->buffer_length > 0){
    new_ptr->sample_buffer = ags_stream_alloc(ptr->buffer_length,
					      ptr->format);
    new_ptr->im_buffer = ags_stream_alloc(ptr->buffer_length,
					  ptr->format);
  }
 ...
 */
gboolean
ags_sf2_synth_util_get_note_256th_mode(AgsSF2SynthUtil *sf2_synth_util)
{
  if(sf2_synth_util == NULL){
    return(0);
  }

  return(sf2_synth_util->note_256th_mode);
}

/**
 * ags_sf2_synth_util_set_note_256th_mode:
 * @sf2_synth_util: the #AgsSF2SynthUtil-struct
 * @note_256th_mode: the note 256th mode
 *
 * Set @note_256th_mode of @sf2_synth_util.
 *
 * Since: 6.1.0
 */
void
ags_sf2_synth_util_set_note_256th_mode(AgsSF2SynthUtil *sf2_synth_util,
				       gboolean note_256th_mode)
{
  ...
  sf2_synth_util->note_256th_mode = note_256th_mode;
}

/**
 * ags_sf2_synth_util_get_offset_256th:
 * @sf2_synth_util: the #AgsSF2SynthUtil-struct
 * 
...
ags_sf2_synth_util_get_offset_256th(AgsSF2SynthUtil *sf2_synth_util)
{
  ...
  return(sf2_synth_util->offset_256th);
}

/**
 * ags_sf2_synth_util_set_offset_256th:
 * @sf2_synth_util: the #AgsSF2SynthUtil-struct
 * @offset_256th: the offset as note 256th
 *
 * Set @offset_256th of @sf2_synth_util.
 *
 * Since: 6.1.0
 */
void
ags_sf2_synth_util_set_offset_256th(AgsSF2SynthUtil *sf2_synth_util,
				    guint offset_256th)
{
  if(sf2_synth_util == NULL){
    return;
  ...
  sf2_synth_util->offset_256th = offset_256th;
}
				   ...
				   gchar *instrument)
{
  ...
  for(i = 0; i < g_list_length(start_list) && i < 128; i++){
	...
	if((IPATCH_SAMPLE_LENDIAN & sf2_sample_format) != 0){
	  sf2_sample_format = ((~IPATCH_SAMPLE_LENDIAN) & orig_sf2_sample_format) | IPATCH_SAMPLE_BENDIAN;
	}
      }else{
	sf2_sample_format = orig_sf2_sample_format;

	if((IPATCH_SAMPLE_BENDIAN & sf2_sample_format) != 0){
	  sf2_sample_format = ((~IPATCH_SAMPLE_BENDIAN) & orig_sf2_sample_format) | IPATCH_SAMPLE_LENDIAN;
	}
      }
	...
	audio_channels = 1;
      ...
      switch(IPATCH_SAMPLE_FORMAT_GET_WIDTH(sf2_sample_format)){
	...
	g_warning("unknown format");
      ...
      buffer =
	sf2_synth_util->sf2_orig_buffer[i] = ags_stream_alloc(MAX(sf2_synth_util->sf2_orig_buffer_length[i], 4096),
							      format);
	...
	sf2_synth_util->sf2_resampled_buffer[i] = ags_stream_alloc(MAX(sf2_synth_util->sf2_resampled_buffer_length[i], 4096),
								   format);
