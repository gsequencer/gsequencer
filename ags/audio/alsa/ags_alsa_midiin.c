...
ags_alsa_midiin_sequencer_interface_init(AgsSequencerInterface *sequencer)
{
  ...
  sequencer->get_frame_clock = ags_alsa_midiin_get_frame_clock;
...
ags_alsa_midiin_init(AgsAlsaMidiin *alsa_midiin)
{
  ...
  /* frame clock */
  alsa_midiin->frame_clock = ags_frame_clock_new();
  ...
  for(i = 0; i < AGS_ALSA_MIDIIN_DEFAULT_APP_BUFFER_SIZE; i++){
    alsa_midiin->app_buffer[i] = (char *) g_malloc(AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE * sizeof(char));

    alsa_midiin->allocated_app_buffer_size[i] = AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE;
  ...
  for(i = 0; i < AGS_ALSA_MIDIIN_DEFAULT_BACKEND_BUFFER_SIZE; i++){
    alsa_midiin->backend_buffer[i] = (char *) g_malloc(AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE * sizeof(char));

    alsa_midiin->allocated_backend_buffer_size[i] = AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE;
			     ...
			     GParamSpec *param_spec)
{
  ...
  case PROP_BPM:
  {
    ...
    ags_frame_clock_set_bpm(alsa_midiin->frame_clock,
			    bpm);
...
ags_alsa_midiin_finalize(GObject *gobject)
{
  ...
  /* frame clock */
  if(alsa_midiin->frame_clock != NULL){
    g_object_unref(alsa_midiin->frame_clock);
  }
...
GObject*
ags_alsa_midiin_get_frame_clock(AgsSequencer *sequencer)
{
  AgsAlsaMidiin *alsa_midiin;
  ...
  /* get alsa midiin mutex */
  alsa_midiin_mutex = AGS_ALSA_MIDIIN_GET_OBJ_MUTEX(alsa_midiin);

  /* set note offset */
  g_rec_mutex_lock(alsa_midiin_mutex);

  frame_clock = (GObject *) alsa_midiin->frame_clock;
			      ...
			      GError **error)
{
  ...
  allocated_backend_buffer_size = alsa_midiin->allocated_backend_buffer_size[backend_buffer_mode];
  backend_buffer_size = 0;
  
  app_buffer = alsa_midiin->app_buffer;

  allocated_app_buffer_size = alsa_midiin->allocated_app_buffer_size[app_buffer_mode];
  app_buffer_size = 0;
  ...
  while(status >= 0){
    ...
    if(status >= 0){
      if(backend_buffer_size + 1 >= allocated_backend_buffer_size){
	if(backend_buffer[backend_buffer_mode] == NULL){
	  ...
	  backend_buffer[backend_buffer_mode] = (char *) g_realloc(backend_buffer[backend_buffer_mode],
								   (allocated_backend_buffer_size + AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE) * sizeof(char));
	}

	memset(backend_buffer[backend_buffer_mode] + allocated_backend_buffer_size, 0, AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE * sizeof(char));

	allocated_backend_buffer_size = 
	  alsa_midiin->allocated_backend_buffer_size[backend_buffer_mode] = allocated_backend_buffer_size + AGS_ALSA_MIDIIN_DEFAULT_BUFFER_SIZE;
      }

      backend_buffer[backend_buffer_mode][backend_buffer_size] = (unsigned char) c;

      backend_buffer_size = 
	alsa_midiin->backend_buffer_size[backend_buffer_mode] = backend_buffer_size + 1;
    }
#endif
  }
  ...
  if(allocated_backend_buffer_size > allocated_app_buffer_size){
    ...
    allocated_app_buffer_size = 
      alsa_midiin->allocated_app_buffer_size[app_buffer_mode] = allocated_backend_buffer_size;	
  }
  ...
  if(backend_buffer_size > 0){
    ...
    memcpy(alsa_midiin->app_buffer[app_buffer_mode], backend_buffer[backend_buffer_mode], app_buffer_size * sizeof(char));
  }
...
ags_alsa_midiin_device_free(AgsSequencer *sequencer)
{
  ...
  /*  */
  ags_frame_clock_stop(alsa_midiin->frame_clock);
...
ags_alsa_midiin_tic(AgsSequencer *sequencer)
{
  ...
  ags_frame_clock_increment_counter(alsa_midiin->frame_clock);

  /* 16th pulse */
  if(ags_frame_clock_get_has_16th_pulse(alsa_midiin->frame_clock)){
    ags_sequencer_offset_changed(sequencer,
				 alsa_midiin->frame_clock->note_offset);
  }
...
ags_alsa_midiin_switch_buffer_flag(AgsAlsaMidiin *alsa_midiin)
{
  ...
  if(alsa_midiin->app_buffer_mode == AGS_ALSA_MIDIIN_APP_BUFFER_0){
    alsa_midiin->app_buffer_mode = AGS_ALSA_MIDIIN_APP_BUFFER_1;

    /* clear buffer */
    memset(alsa_midiin->app_buffer[3], 0, alsa_midiin->app_buffer_size[3] * sizeof(char));
    
    alsa_midiin->app_buffer_size[3] = 0;
  }else if(alsa_midiin->app_buffer_mode == AGS_ALSA_MIDIIN_APP_BUFFER_1){
    alsa_midiin->app_buffer_mode = AGS_ALSA_MIDIIN_APP_BUFFER_2;

    /* clear buffer */
    memset(alsa_midiin->app_buffer[0], 0, alsa_midiin->app_buffer_size[0] * sizeof(char));
    
    alsa_midiin->app_buffer_size[0] = 0;
  }else if(alsa_midiin->app_buffer_mode == AGS_ALSA_MIDIIN_APP_BUFFER_2){
    alsa_midiin->app_buffer_mode = AGS_ALSA_MIDIIN_APP_BUFFER_3;

    /* clear buffer */
    memset(alsa_midiin->app_buffer[1], 0, alsa_midiin->app_buffer_size[1] * sizeof(char));
    
    alsa_midiin->app_buffer_size[1] = 0;
  }else if(alsa_midiin->app_buffer_mode == AGS_ALSA_MIDIIN_APP_BUFFER_3){
    alsa_midiin->app_buffer_mode = AGS_ALSA_MIDIIN_APP_BUFFER_0;

    /* clear buffer */
    memset(alsa_midiin->app_buffer[2], 0, alsa_midiin->app_buffer_size[2] * sizeof(char));
    
    alsa_midiin->app_buffer_size[2] = 0;
  }
