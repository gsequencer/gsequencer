...
static void ags_audio_unit_server_class_init(AgsAudioUnitServerClass *audio_unit_server);
static void ags_audio_unit_server_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_audio_unit_server_sound_server_interface_init(AgsSoundServerInterface *sound_server);
static void ags_audio_unit_server_init(AgsAudioUnitServer *audio_unit_server);
static void ags_audio_unit_server_set_property(GObject *gobject,
					       guint prop_id,
					       const GValue *value,
					       GParamSpec *param_spec);
static void ags_audio_unit_server_get_property(GObject *gobject,
					       guint prop_id,
					       GValue *value,
					       GParamSpec *param_spec);
static void ags_audio_unit_server_dispose(GObject *gobject);
static void ags_audio_unit_server_finalize(GObject *gobject);

static AgsUUID* ags_audio_unit_server_get_uuid(AgsConnectable *connectable);
static gboolean ags_audio_unit_server_has_resource(AgsConnectable *connectable);
static gboolean ags_audio_unit_server_is_ready(AgsConnectable *connectable);
static void ags_audio_unit_server_add_to_registry(AgsConnectable *connectable);
static void ags_audio_unit_server_remove_from_registry(AgsConnectable *connectable);
static xmlNode* ags_audio_unit_server_list_resource(AgsConnectable *connectable);
static xmlNode* ags_audio_unit_server_xml_compose(AgsConnectable *connectable);
static void ags_audio_unit_server_xml_parse(AgsConnectable *connectable,
					    xmlNode *node);
static gboolean ags_audio_unit_server_is_connected(AgsConnectable *connectable);
static void ags_audio_unit_server_connect(AgsConnectable *connectable);
static void ags_audio_unit_server_disconnect(AgsConnectable *connectable);

static void ags_audio_unit_server_set_url(AgsSoundServer *sound_server,
					  gchar *url);
static gchar* ags_audio_unit_server_get_url(AgsSoundServer *sound_server);
static void ags_audio_unit_server_set_ports(AgsSoundServer *sound_server,
					    guint *ports, guint port_count);
static guint* ags_audio_unit_server_get_ports(AgsSoundServer *sound_server,
					      guint *port_count);
static void ags_audio_unit_server_set_soundcard(AgsSoundServer *sound_server,
						gchar *client_uuid,
						GList *soundcard);
static GList* ags_audio_unit_server_get_soundcard(AgsSoundServer *sound_server,
						  gchar *client_uuid);
static void ags_audio_unit_server_set_sequencer(AgsSoundServer *sound_server,
						gchar *client_uuid,
						GList *sequencer);
static GList* ags_audio_unit_server_get_sequencer(AgsSoundServer *sound_server,
						  gchar *client_uuid);
static GObject* ags_audio_unit_server_register_soundcard(AgsSoundServer *sound_server,
							 gboolean is_output);
static void ags_audio_unit_server_unregister_soundcard(AgsSoundServer *sound_server,
						       GObject *soundcard);
static GObject* ags_audio_unit_server_register_sequencer(AgsSoundServer *sound_server,
							 gboolean is_output);
...
ags_audio_unit_server_get_type()
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_audio_unit_server);
  }
