...
static void ags_core_audio_devin_class_init(AgsCoreAudioDevinClass *core_audio_devin);
static void ags_core_audio_devin_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_core_audio_devin_soundcard_interface_init(AgsSoundcardInterface *soundcard);
static void ags_core_audio_devin_init(AgsCoreAudioDevin *core_audio_devin);
static void ags_core_audio_devin_set_property(GObject *gobject,
					      guint prop_id,
					      const GValue *value,
					      GParamSpec *param_spec);
static void ags_core_audio_devin_get_property(GObject *gobject,
					      guint prop_id,
					      GValue *value,
					      GParamSpec *param_spec);
static void ags_core_audio_devin_dispose(GObject *gobject);
static void ags_core_audio_devin_finalize(GObject *gobject);

static AgsUUID* ags_core_audio_devin_get_uuid(AgsConnectable *connectable);
static gboolean ags_core_audio_devin_has_resource(AgsConnectable *connectable);
static gboolean ags_core_audio_devin_is_ready(AgsConnectable *connectable);
static void ags_core_audio_devin_add_to_registry(AgsConnectable *connectable);
static void ags_core_audio_devin_remove_from_registry(AgsConnectable *connectable);
static xmlNode* ags_core_audio_devin_list_resource(AgsConnectable *connectable);
static xmlNode* ags_core_audio_devin_xml_compose(AgsConnectable *connectable);
static void ags_core_audio_devin_xml_parse(AgsConnectable *connectable,
					   xmlNode *node);
static gboolean ags_core_audio_devin_is_connected(AgsConnectable *connectable);
static void ags_core_audio_devin_connect(AgsConnectable *connectable);
static void ags_core_audio_devin_disconnect(AgsConnectable *connectable);

static void ags_core_audio_devin_set_device(AgsSoundcard *soundcard,
					    gchar *device);
static gchar* ags_core_audio_devin_get_device(AgsSoundcard *soundcard);

static void ags_core_audio_devin_set_presets(AgsSoundcard *soundcard,
					     guint channels,
					     guint rate,
					     guint buffer_size,
					     AgsSoundcardFormat format);
static void ags_core_audio_devin_get_presets(AgsSoundcard *soundcard,
					     guint *channels,
					     guint *rate,
					     guint *buffer_size,
					     AgsSoundcardFormat *format);

static void ags_core_audio_devin_list_cards(AgsSoundcard *soundcard,
					    GList **card_id, GList **card_name);
static void ags_core_audio_devin_pcm_info(AgsSoundcard *soundcard, gchar *card_id,
					  guint *channels_min, guint *channels_max,
					  guint *rate_min, guint *rate_max,
					  guint *buffer_size_min, guint *buffer_size_max,
					  GError **error);
static guint ags_core_audio_devin_get_capability(AgsSoundcard *soundcard);

static gboolean ags_core_audio_devin_is_starting(AgsSoundcard *soundcard);
static gboolean ags_core_audio_devin_is_recording(AgsSoundcard *soundcard);

static gchar* ags_core_audio_devin_get_uptime(AgsSoundcard *soundcard);

static void ags_core_audio_devin_port_init(AgsSoundcard *soundcard,
					   GError **error);
static void ags_core_audio_devin_port_record(AgsSoundcard *soundcard,
					     GError **error);
static void ags_core_audio_devin_port_free(AgsSoundcard *soundcard);

static void ags_core_audio_devin_tic(AgsSoundcard *soundcard);
static void ags_core_audio_devin_offset_changed(AgsSoundcard *soundcard,
						guint64 note_offset);

static void ags_core_audio_devin_set_bpm(AgsSoundcard *soundcard,
					 gdouble bpm);
static gdouble ags_core_audio_devin_get_bpm(AgsSoundcard *soundcard);

static void ags_core_audio_devin_set_start_note_offset(AgsSoundcard *soundcard,
						       guint64 start_note_offset);
static guint64 ags_core_audio_devin_get_start_note_offset(AgsSoundcard *soundcard);

static GObject* ags_core_audio_devin_get_frame_clock(AgsSoundcard *soundcard);

static void* ags_core_audio_devin_get_buffer(AgsSoundcard *soundcard);
static void* ags_core_audio_devin_get_next_buffer(AgsSoundcard *soundcard);
static void* ags_core_audio_devin_get_prev_buffer(AgsSoundcard *soundcard);

static void ags_core_audio_devin_lock_buffer(AgsSoundcard *soundcard,
					     void *buffer);
static void ags_core_audio_devin_unlock_buffer(AgsSoundcard *soundcard,
					       void *buffer);
...
ags_core_audio_devin_get_uptime(AgsSoundcard *soundcard)
{
  ...
  uptime = ags_frame_clock_to_time_string(core_audio_devin->frame_clock);
...
ags_core_audio_devin_port_free(AgsSoundcard *soundcard)
{
  ...
  /*  */
  ags_frame_clock_stop(core_audio_devin->frame_clock);
