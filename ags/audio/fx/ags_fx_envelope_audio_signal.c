...
static void ags_fx_envelope_audio_signal_class_init(AgsFxEnvelopeAudioSignalClass *fx_envelope_audio_signal);
static void ags_fx_envelope_audio_signal_init(AgsFxEnvelopeAudioSignal *fx_envelope_audio_signal);
static void ags_fx_envelope_audio_signal_dispose(GObject *gobject);
static void ags_fx_envelope_audio_signal_finalize(GObject *gobject);

static gdouble ags_fx_envelope_audio_signal_get_ratio(guint x0, gdouble y0,
						      guint x1, gdouble y1);
static gdouble ags_fx_envelope_audio_signal_get_volume(gdouble volume, gdouble ratio,
						       guint start_x, guint current_x,
						       guint length);

static void ags_fx_envelope_audio_signal_real_run_inter(AgsRecall *recall);
...
ags_fx_envelope_audio_signal_get_type()
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_fx_envelope_audio_signal);
  }
