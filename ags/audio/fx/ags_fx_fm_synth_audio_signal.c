...
static void ags_fx_fm_synth_audio_signal_class_init(AgsFxFMSynthAudioSignalClass *fx_fm_synth_audio_signal);
static void ags_fx_fm_synth_audio_signal_init(AgsFxFMSynthAudioSignal *fx_fm_synth_audio_signal);
static void ags_fx_fm_synth_audio_signal_dispose(GObject *gobject);
static void ags_fx_fm_synth_audio_signal_finalize(GObject *gobject);

static void ags_fx_fm_synth_audio_signal_stream_feed(AgsFxNotationAudioSignal *fx_notation_audio_signal,
...
static void ags_fx_fm_synth_audio_signal_notify_remove(AgsFxNotationAudioSignal *fx_notation_audio_signal,
						       AgsAudioSignal *source,
						       AgsNote *note,
						       guint x0, guint x1,
						       guint y);
...
static const gchar *ags_fx_fm_synth_audio_signal_plugin_name = "ags-fx-fm-synth";
					 ...
					 gdouble delay, guint buffer_size)
{
  ...
  ags_frame_clock_get_note_256th_offset(AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_fm_synth_audio_processor)->frame_clock,
					&(note_256th_offset[0]),
					&note_256th_offset_length);
 
  note_256th_offset_lower = note_256th_offset[0];

  note_256th_delay = (gdouble) AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_fm_synth_audio_processor)->frame_clock->absolute_delay / 16.0;
