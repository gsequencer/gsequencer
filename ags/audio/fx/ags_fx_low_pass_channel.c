...
static void ags_fx_low_pass_channel_class_init(AgsFxLowPassChannelClass *fx_low_pass_channel);
static void ags_fx_low_pass_channel_init(AgsFxLowPassChannel *fx_low_pass_channel);
static void ags_fx_low_pass_channel_set_property(GObject *gobject,
						 guint prop_id,
						 const GValue *value,
						 GParamSpec *param_spec);
static void ags_fx_low_pass_channel_get_property(GObject *gobject,
						 guint prop_id,
						 GValue *value,
						 GParamSpec *param_spec);
static void ags_fx_low_pass_channel_dispose(GObject *gobject);
static void ags_fx_low_pass_channel_finalize(GObject *gobject);

static AgsPluginPort* ags_fx_low_pass_channel_get_enabled_plugin_port();
...
ags_fx_low_pass_channel_get_type()
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_fx_low_pass_channel);
  }
...
ags_fx_low_pass_channel_init(AgsFxLowPassChannel *fx_low_pass_channel)
{
  ...
  fx_low_pass_channel->enabled->port_value.ags_port_float = (gfloat) FALSE;

  g_object_set(fx_low_pass_channel->enabled,
	       "plugin-port", ags_fx_low_pass_channel_get_enabled_plugin_port(),
	       NULL);

  ags_recall_add_port((AgsRecall *) fx_low_pass_channel,
		      fx_low_pass_channel->enabled);
				     ...
				     GParamSpec *param_spec)
{
  ...
  case PROP_ENABLED:
  {
    ...
    if(port == fx_low_pass_channel->enabled){
      g_rec_mutex_unlock(recall_mutex);	

      return;
    }

    if(fx_low_pass_channel->enabled != NULL){
      g_object_unref(G_OBJECT(fx_low_pass_channel->enabled));
    }
    ...
    fx_low_pass_channel->enabled = port;
				     ...
				     GParamSpec *param_spec)
{
  ...
  case PROP_ENABLED:
  {
    ...
    g_value_set_object(value, fx_low_pass_channel->enabled);
...
ags_fx_low_pass_channel_dispose(GObject *gobject)
{
  ...
  if(fx_low_pass_channel->enabled != NULL){
    ...
    fx_low_pass_channel->enabled = NULL;
  }  
...
ags_fx_low_pass_channel_get_enabled_plugin_port()
{
