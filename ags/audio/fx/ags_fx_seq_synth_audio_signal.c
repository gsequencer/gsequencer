...
static void ags_fx_seq_synth_audio_signal_class_init(AgsFxSeqSynthAudioSignalClass *fx_seq_synth_audio_signal);
static void ags_fx_seq_synth_audio_signal_init(AgsFxSeqSynthAudioSignal *fx_seq_synth_audio_signal);
static void ags_fx_seq_synth_audio_signal_dispose(GObject *gobject);
static void ags_fx_seq_synth_audio_signal_finalize(GObject *gobject);

static void ags_fx_seq_synth_audio_signal_stream_feed(AgsFxNotationAudioSignal *fx_notation_audio_signal,
...
static void ags_fx_seq_synth_audio_signal_notify_remove(AgsFxNotationAudioSignal *fx_notation_audio_signal,
							AgsAudioSignal *source,
							AgsNote *note,
							guint x0, guint x1,
							guint y);
...
/**
 * SECTION:ags_fx_seq_synth_audio_signal
 * @short_description: fx seq synth audio signal
 * @title: AgsFxSeqSynthAudioSignal
 * @section_id:
 * @include: ags/audio/fx/ags_fx_seq_synth_audio_signal.h
 *
...
ags_fx_seq_synth_audio_signal_get_type()
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    GType ags_type_fx_seq_synth_audio_signal = 0;

    static const GTypeInfo ags_fx_seq_synth_audio_signal_info = {
      sizeof (AgsFxSeqSynthAudioSignalClass),
      NULL, /* base_init */
      NULL, /* base_finalize */
      (GClassInitFunc) ags_fx_seq_synth_audio_signal_class_init,
      NULL, /* class_finalize */
      NULL, /* class_audio_signal */
      sizeof (AgsFxSeqSynthAudioSignal),
      0,    /* n_preallocs */
      (GInstanceInitFunc) ags_fx_seq_synth_audio_signal_init,
    };

    ags_type_fx_seq_synth_audio_signal = g_type_register_static(AGS_TYPE_FX_NOTATION_AUDIO_SIGNAL,
								"AgsFxSeqSynthAudioSignal",
								&ags_fx_seq_synth_audio_signal_info,
								0);

    g_once_init_leave(&g_define_type_id__static, ags_type_fx_seq_synth_audio_signal);
  }
...
void
ags_fx_seq_synth_audio_signal_class_init(AgsFxSeqSynthAudioSignalClass *fx_seq_synth_audio_signal)
{
  GObjectClass *gobject;
  AgsFxNotationAudioSignalClass *fx_notation_audio_signal;

  ags_fx_seq_synth_audio_signal_parent_class = g_type_class_peek_parent(fx_seq_synth_audio_signal);

  /* GObjectClass */
  gobject = (GObjectClass *) fx_seq_synth_audio_signal;

  gobject->dispose = ags_fx_seq_synth_audio_signal_dispose;
  gobject->finalize = ags_fx_seq_synth_audio_signal_finalize;

  /* AgsFxNotationAudioSignalClass */
  fx_notation_audio_signal = (AgsFxNotationAudioSignalClass *) fx_seq_synth_audio_signal;
  
  fx_notation_audio_signal->stream_feed = ags_fx_seq_synth_audio_signal_stream_feed;
  fx_notation_audio_signal->notify_remove = ags_fx_seq_synth_audio_signal_notify_remove;
}

void
ags_fx_seq_synth_audio_signal_init(AgsFxSeqSynthAudioSignal *fx_seq_synth_audio_signal)
{
  AGS_RECALL(fx_seq_synth_audio_signal)->name = "ags-fx-seq-synth";
  AGS_RECALL(fx_seq_synth_audio_signal)->version = AGS_RECALL_DEFAULT_VERSION;
  AGS_RECALL(fx_seq_synth_audio_signal)->build_id = AGS_RECALL_DEFAULT_BUILD_ID;
  AGS_RECALL(fx_seq_synth_audio_signal)->xml_type = "ags-fx-seq-synth-audio-signal";
}

void
ags_fx_seq_synth_audio_signal_dispose(GObject *gobject)
{
  AgsFxSeqSynthAudioSignal *fx_seq_synth_audio_signal;
  
  fx_seq_synth_audio_signal = AGS_FX_SEQ_SYNTH_AUDIO_SIGNAL(gobject);
  
  /* call parent */
  G_OBJECT_CLASS(ags_fx_seq_synth_audio_signal_parent_class)->dispose(gobject);
}

void
ags_fx_seq_synth_audio_signal_finalize(GObject *gobject)
{
  AgsFxSeqSynthAudioSignal *fx_seq_synth_audio_signal;
  ...
  /* call parent */
  G_OBJECT_CLASS(ags_fx_seq_synth_audio_signal_parent_class)->finalize(gobject);
}

void
ags_fx_seq_synth_audio_signal_stream_feed(AgsFxNotationAudioSignal *fx_notation_audio_signal,
					  AgsAudioSignal *source,
					  AgsNote *note,
					  gboolean pattern_mode,
					  guint x0, guint x1,
					  ...
					  gdouble delay, guint buffer_size)
{
  ...
  AgsFxSeqSynthRecycling *fx_seq_synth_recycling;
  AgsFxSeqSynthAudioSignal *fx_seq_synth_audio_signal;
  ...
  GRecMutex *fx_seq_synth_audio_processor_mutex;

  fx_seq_synth_audio_signal = (AgsFxSeqSynthAudioSignal *) fx_notation_audio_signal;
  ...
  fx_seq_synth_channel_processor = NULL;

  fx_seq_synth_recycling = NULL;
  ...
  note_256th_mode = ags_fx_notation_audio_get_note_256th_mode((AgsFxNotationAudio *) fx_seq_synth_audio);
  ...
  g_object_get(fx_seq_synth_recycling,
	       "parent", &fx_seq_synth_channel_processor,
	       NULL);
  
  g_object_get(fx_seq_synth_channel_processor,
  ...
  g_object_get(fx_seq_synth_audio_processor,
  ...
  audio_buffer_util_format = ags_audio_buffer_util_format_from_soundcard(&(fx_seq_synth_audio_signal->audio_buffer_util),
									 format);

  copy_mode = ags_audio_buffer_util_get_copy_mode_from_format(&(fx_seq_synth_audio_signal->audio_buffer_util),
							      audio_buffer_util_format,
							      audio_buffer_util_format);
  
  /* get synth mutex */
  fx_seq_synth_audio_mutex = AGS_RECALL_GET_OBJ_MUTEX(fx_seq_synth_audio);
  fx_seq_synth_audio_processor_mutex = AGS_RECALL_GET_OBJ_MUTEX(fx_seq_synth_audio_processor);

  g_rec_mutex_lock(fx_seq_synth_audio_processor_mutex);

  note_256th_offset_length = 0;
    
  ags_frame_clock_get_note_256th_offset(AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_seq_synth_audio_processor)->frame_clock,
					&(note_256th_offset[0]),
					&note_256th_offset_length);
  
  note_256th_offset_lower = note_256th_offset[0];

  note_256th_delay = (gdouble) AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_seq_synth_audio_processor)->frame_clock->absolute_delay / 16.0;
  
  g_rec_mutex_unlock(fx_seq_synth_audio_processor_mutex);
  ...
  if(fx_seq_synth_audio != NULL){
    ...
    channel_data = fx_seq_synth_audio->scope_data[sound_scope]->channel_data[audio_channel];
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_synth_oscillator_mode(channel_data->seq_synth_0,
    ...
    ags_seq_synth_util_set_frequency(channel_data->seq_synth_0,
				     exp2(((octave * 12.0) + key + ((gdouble) midi_note - 48.0)) / 12.0) * 440.0);

    //    g_message("synth 0 - osc = %d, frequency = %f, octave = %f, key= %f", channel_data->seq_synth_0.synth_oscillator_mode, channel_data->seq_synth_0.frequency, octave, key);
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_phase(channel_data->seq_synth_0,
				   ((gdouble) samplerate / ags_seq_synth_util_get_frequency(channel_data->seq_synth_0)) * ((gdouble) g_value_get_float(&value) / (2.0 * M_PI)));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_lfo_oscillator_mode(channel_data->seq_synth_0,
						 (guint) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_lfo_frequency(channel_data->seq_synth_0,
					   (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_lfo_depth(channel_data->seq_synth_0,
				       (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_tuning(channel_data->seq_synth_0,
				    (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_synth_oscillator_mode(channel_data->seq_synth_1,
						   (guint) g_value_get_float(&value));
    ...
    ags_seq_synth_util_set_frequency(channel_data->seq_synth_1,
				     exp2(((octave * 12.0) + key + ((gdouble) midi_note - 48.0)) / 12.0) * 440.0);

    //    g_message("synth 2 - osc = %d, frequency = %f, octave = %f, key= %f", channel_data->seq_synth_1.synth_oscillator_mode, channel_data->seq_synth_1.frequency, octave, key);
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_phase(channel_data->seq_synth_1,
				   ((gdouble) samplerate / ags_seq_synth_util_get_frequency(channel_data->seq_synth_1)) * ((gdouble) g_value_get_float(&value) / (2.0 * M_PI)));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_lfo_oscillator_mode(channel_data->seq_synth_1,
						 (guint) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_lfo_frequency(channel_data->seq_synth_1,
					   (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_lfo_depth(channel_data->seq_synth_1,
				       (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_seq_synth_util_set_tuning(channel_data->seq_synth_1,
				    (gdouble) g_value_get_float(&value));
  ...
  copy_mode_out = ags_audio_buffer_util_get_copy_mode_from_format(&(fx_seq_synth_audio_signal->audio_buffer_util),
								  ags_audio_buffer_util_format_from_soundcard(&(fx_seq_synth_audio_signal->audio_buffer_util),
													      format),
  ...
  if(midi_note >= 0 &&
    ...
    g_rec_mutex_lock(fx_seq_synth_audio_mutex);

    scope_data = fx_seq_synth_audio->scope_data[sound_scope];

    channel_data = scope_data->channel_data[audio_channel];

    input_data = channel_data->input_data[midi_note];

    g_rec_mutex_unlock(fx_seq_synth_audio_mutex);
    ...
    /* seq synth 0 */
    ags_seq_synth_util_set_source(channel_data->seq_synth_0,
				  source->stream_current->data);
    ags_seq_synth_util_set_source_stride(channel_data->seq_synth_0,
					 1);
    ...
    ags_seq_synth_util_set_vibrato_gain(channel_data->seq_synth_0,
					vibrato_gain);
    
    ags_seq_synth_util_set_vibrato_lfo_depth(channel_data->seq_synth_0,
					     vibrato_lfo_depth);

    ags_seq_synth_util_set_vibrato_lfo_freq(channel_data->seq_synth_0,
					    vibrato_lfo_freq);

    ags_seq_synth_util_set_vibrato_tuning(channel_data->seq_synth_0,
					  vibrato_tuning);
    
    if(!note_256th_mode){
      ags_seq_synth_util_set_frame_count(channel_data->seq_synth_0,
					 floor(((offset_counter - x0) * delay + delay_counter + 1.0) * buffer_size));

      ags_seq_synth_util_set_offset(channel_data->seq_synth_0,
      ...
      ags_seq_synth_util_set_offset_256th(channel_data->seq_synth_0,
    ...
    if(synth_0_sync_enabled){
      ...
      offset = ags_seq_synth_util_get_offset(channel_data->seq_synth_0);
      
      freq_period = samplerate / ags_seq_synth_util_get_frequency(channel_data->seq_synth_0);
	...
	//	g_message("synth 0 - offset = %d, i = %d, i_reset = %d, sync_reset = %d", channel_data->seq_synth_0.offset, i, i_reset, sync_reset);
	
	if(i >= ags_seq_synth_util_get_offset(channel_data->seq_synth_0)){
	  ags_seq_synth_util_set_offset(channel_data->seq_synth_0,
					i);
	  ...
	  switch(ags_seq_synth_util_get_synth_oscillator_mode(channel_data->seq_synth_0)){
	  case AGS_SYNTH_OSCILLATOR_SIN:
	    {
	      ags_seq_synth_util_compute_sin(channel_data->seq_synth_0);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_SAWTOOTH:
	    {
	      ags_seq_synth_util_compute_sawtooth(channel_data->seq_synth_0);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_TRIANGLE:
	    {
	      ags_seq_synth_util_compute_triangle(channel_data->seq_synth_0);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_SQUARE:
	    {
	      ags_seq_synth_util_compute_square(channel_data->seq_synth_0);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_IMPULSE:
	    {
	      ags_seq_synth_util_compute_impulse(channel_data->seq_synth_0);
	    }
	    break;
	  }
    ...
    /* seq synth 1 */
    ags_seq_synth_util_set_source(channel_data->seq_synth_1,
				  source->stream_current->data);
    ags_seq_synth_util_set_source_stride(channel_data->seq_synth_1,
					 1);
    ...
    ags_seq_synth_util_set_vibrato_gain(channel_data->seq_synth_1,
					vibrato_gain);
    
    ags_seq_synth_util_set_vibrato_lfo_depth(channel_data->seq_synth_1,
					     vibrato_lfo_depth);

    ags_seq_synth_util_set_vibrato_lfo_freq(channel_data->seq_synth_1,
					    vibrato_lfo_freq);

    ags_seq_synth_util_set_vibrato_tuning(channel_data->seq_synth_1,
					  vibrato_tuning);

    if(!note_256th_mode){
      ags_seq_synth_util_set_frame_count(channel_data->seq_synth_1,
					 floor(((offset_counter - x0) * delay + delay_counter + 1.0) * buffer_size));

      ags_seq_synth_util_set_offset(channel_data->seq_synth_1,
				    floor(((offset_counter - x0) * delay + delay_counter) * buffer_size));
    }else{
      ags_seq_synth_util_set_frame_count(channel_data->seq_synth_1,
					 (guint) floor((double) (note_256th_offset_lower - x0_256th) * note_256th_delay * (double) buffer_size) + (guint) floor(delay * (double) buffer_size));
      ...
      ags_seq_synth_util_set_offset_256th(channel_data->seq_synth_1,
					  (guint) floor((double) (note_256th_offset_lower - x0_256th) * note_256th_delay * (double) buffer_size));
    }
    ...
    if(synth_1_sync_enabled){
      ...
      offset = ags_seq_synth_util_get_offset(channel_data->seq_synth_1);
      
      freq_period = samplerate / ags_seq_synth_util_get_frequency(channel_data->seq_synth_1);
	...
	//	g_message("synth 1 - offset = %d, i = %d, i_reset = %d, sync_reset = %d", channel_data->seq_synth_1.offset, i, i_reset, sync_reset);
	
	if(i >= ags_seq_synth_util_get_offset(channel_data->seq_synth_1)){
	  ags_seq_synth_util_set_offset(channel_data->seq_synth_1,
					i);
	  ...
	  switch(ags_seq_synth_util_get_synth_oscillator_mode(channel_data->seq_synth_1)){
	  case AGS_SYNTH_OSCILLATOR_SIN:
	    {
	      ags_seq_synth_util_compute_sin(channel_data->seq_synth_1);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_SAWTOOTH:
	    {
	      ags_seq_synth_util_compute_sawtooth(channel_data->seq_synth_1);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_TRIANGLE:
	    {
	      ags_seq_synth_util_compute_triangle(channel_data->seq_synth_1);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_SQUARE:
	    {
	      ags_seq_synth_util_compute_square(channel_data->seq_synth_1);
	    }
	    break;
	  case AGS_SYNTH_OSCILLATOR_IMPULSE:
	    {
	      ags_seq_synth_util_compute_impulse(channel_data->seq_synth_1);
	    }
	    break;
	  }
  ...
  if(fx_seq_synth_channel_processor != NULL){
    g_object_unref(fx_seq_synth_channel_processor);
  }
  
  if(fx_seq_synth_recycling != NULL){
    g_object_unref(fx_seq_synth_recycling);
  }
}

void
ags_fx_seq_synth_audio_signal_notify_remove(AgsFxNotationAudioSignal *fx_notation_audio_signal,
					    AgsAudioSignal *source,
					    AgsNote *note,
					    guint x0, guint x1,
					    guint y)
 ...
 *
 * Returns: the new #AgsFxSeqSynthAudioSignal
 *
 * Since: 7.5.0
 */
AgsFxSeqSynthAudioSignal*
ags_fx_seq_synth_audio_signal_new(AgsAudioSignal *audio_signal)
{
  ...
  fx_seq_synth_audio_signal = (AgsFxSeqSynthAudioSignal *) g_object_new(AGS_TYPE_FX_SEQ_SYNTH_AUDIO_SIGNAL,
									"source", audio_signal,
									NULL);

  return(fx_seq_synth_audio_signal);
}
