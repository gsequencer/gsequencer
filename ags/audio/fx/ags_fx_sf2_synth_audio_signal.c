...
static void ags_fx_sf2_synth_audio_signal_class_init(AgsFxSF2SynthAudioSignalClass *fx_sf2_synth_audio_signal);
static void ags_fx_sf2_synth_audio_signal_init(AgsFxSF2SynthAudioSignal *fx_sf2_synth_audio_signal);
static void ags_fx_sf2_synth_audio_signal_dispose(GObject *gobject);
static void ags_fx_sf2_synth_audio_signal_finalize(GObject *gobject);

static void ags_fx_sf2_synth_audio_signal_stream_feed(AgsFxNotationAudioSignal *fx_notation_audio_signal,
...
static void ags_fx_sf2_synth_audio_signal_notify_remove(AgsFxNotationAudioSignal *fx_notation_audio_signal,
							AgsAudioSignal *source,
							AgsNote *note,
							guint x0, guint x1,
							guint y);
...
ags_fx_sf2_synth_audio_signal_get_type()
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_fx_sf2_synth_audio_signal);
  }
					  ...
					  gdouble delay, guint buffer_size)
{
  ...
  fx_sf2_synth_audio_signal = (AgsFxSF2SynthAudioSignal *) fx_notation_audio_signal;
  ...
  audio_buffer_util_format = ags_audio_buffer_util_format_from_soundcard(&(fx_sf2_synth_audio_signal->audio_buffer_util),
									 format);

  copy_mode = ags_audio_buffer_util_get_copy_mode_from_format(&(fx_sf2_synth_audio_signal->audio_buffer_util),
  ...
  ags_frame_clock_get_note_256th_offset(AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_sf2_synth_audio_processor)->frame_clock,
					&(note_256th_offset[0]),
					&note_256th_offset_length);
  
  note_256th_offset_lower = note_256th_offset[0];

  note_256th_delay = (gdouble) AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_sf2_synth_audio_processor)->frame_clock->absolute_delay / 16.0;
  ...
  if(fx_sf2_synth_audio != NULL){
    ...
    if(port != NULL){      
      ...
      ags_low_pass_filter_util_set_cut_off_frequency(channel_data->low_pass_filter_util,
						     (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_low_pass_filter_util_set_filter_gain(channel_data->low_pass_filter_util,
					       (gdouble) g_value_get_float(&value));
    ...
    if(port != NULL){      
      ...
      ags_low_pass_filter_util_set_no_clip(channel_data->low_pass_filter_util,
					   (gdouble) g_value_get_float(&value));
  ...
  if(midi_note >= 0 &&
    ...
    ags_audio_buffer_util_clear_buffer(&(fx_sf2_synth_audio_signal->audio_buffer_util),
    ...
    if(ags_chorus_util_get_depth(channel_data->chorus_util) != 0.0 &&
      ...
      ags_audio_buffer_util_copy_buffer_to_buffer(&(fx_sf2_synth_audio_signal->audio_buffer_util),
