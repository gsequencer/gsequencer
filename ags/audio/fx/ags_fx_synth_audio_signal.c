...
static void ags_fx_synth_audio_signal_class_init(AgsFxSynthAudioSignalClass *fx_synth_audio_signal);
static void ags_fx_synth_audio_signal_init(AgsFxSynthAudioSignal *fx_synth_audio_signal);
static void ags_fx_synth_audio_signal_dispose(GObject *gobject);
static void ags_fx_synth_audio_signal_finalize(GObject *gobject);

static void ags_fx_synth_audio_signal_stream_feed(AgsFxNotationAudioSignal *fx_notation_audio_signal,
...
static void ags_fx_synth_audio_signal_notify_remove(AgsFxNotationAudioSignal *fx_notation_audio_signal,
						    AgsAudioSignal *source,
						    AgsNote *note,
						    guint x0, guint x1,
						    guint y);
...
static const gchar *ags_fx_synth_audio_signal_plugin_name = "ags-fx-synth";
...
ags_fx_synth_audio_signal_get_type()
{
  ...
  if(g_once_init_enter(&g_define_type_id__static)){
    ...
    g_once_init_leave(&g_define_type_id__static, ags_type_fx_synth_audio_signal);
  }
				      ...
				      gdouble delay, guint buffer_size)
{
  ...
  note_256th_mode = ags_fx_notation_audio_get_note_256th_mode((AgsFxNotationAudio *) fx_synth_audio);
  ...
  fx_synth_audio_processor_mutex = AGS_RECALL_GET_OBJ_MUTEX(fx_synth_audio_processor);

  g_rec_mutex_lock(fx_synth_audio_processor_mutex);

  note_256th_offset_length = 0;
    
  ags_frame_clock_get_note_256th_offset(AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_synth_audio_processor)->frame_clock,
					&(note_256th_offset[0]),
					&note_256th_offset_length);
  
  note_256th_offset_lower = note_256th_offset[0];

  note_256th_delay = (gdouble) AGS_FX_NOTATION_AUDIO_PROCESSOR(fx_synth_audio_processor)->frame_clock->absolute_delay / 16.0;
