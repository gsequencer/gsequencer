...
static void ags_jack_devout_class_init(AgsJackDevoutClass *jack_devout);
static void ags_jack_devout_connectable_interface_init(AgsConnectableInterface *connectable);
static void ags_jack_devout_soundcard_interface_init(AgsSoundcardInterface *soundcard);
static void ags_jack_devout_init(AgsJackDevout *jack_devout);
static void ags_jack_devout_set_property(GObject *gobject,
					 guint prop_id,
					 const GValue *value,
					 GParamSpec *param_spec);
static void ags_jack_devout_get_property(GObject *gobject,
					 guint prop_id,
					 GValue *value,
					 GParamSpec *param_spec);
static void ags_jack_devout_dispose(GObject *gobject);
static void ags_jack_devout_finalize(GObject *gobject);

static AgsUUID* ags_jack_devout_get_uuid(AgsConnectable *connectable);
static gboolean ags_jack_devout_has_resource(AgsConnectable *connectable);
static gboolean ags_jack_devout_is_ready(AgsConnectable *connectable);
static void ags_jack_devout_add_to_registry(AgsConnectable *connectable);
static void ags_jack_devout_remove_from_registry(AgsConnectable *connectable);
static xmlNode* ags_jack_devout_list_resource(AgsConnectable *connectable);
static xmlNode* ags_jack_devout_xml_compose(AgsConnectable *connectable);
static void ags_jack_devout_xml_parse(AgsConnectable *connectable,
				      xmlNode *node);
static gboolean ags_jack_devout_is_connected(AgsConnectable *connectable);
static void ags_jack_devout_connect(AgsConnectable *connectable);
static void ags_jack_devout_disconnect(AgsConnectable *connectable);

static void ags_jack_devout_set_device(AgsSoundcard *soundcard,
				       gchar *device);
static gchar* ags_jack_devout_get_device(AgsSoundcard *soundcard);

static void ags_jack_devout_set_presets(AgsSoundcard *soundcard,
					guint channels,
					guint rate,
					guint buffer_size,
					AgsSoundcardFormat format);
static void ags_jack_devout_get_presets(AgsSoundcard *soundcard,
					guint *channels,
					guint *rate,
					guint *buffer_size,
					AgsSoundcardFormat *format);

static void ags_jack_devout_list_cards(AgsSoundcard *soundcard,
				       GList **card_id, GList **card_name);
static void ags_jack_devout_pcm_info(AgsSoundcard *soundcard, gchar *card_id,
				     guint *channels_min, guint *channels_max,
				     guint *rate_min, guint *rate_max,
				     guint *buffer_size_min, guint *buffer_size_max,
				     GError **error);
static guint ags_jack_devout_get_capability(AgsSoundcard *soundcard);

static void ags_jack_devout_set_bpm(AgsSoundcard *soundcard,
				    gdouble bpm);
static gdouble ags_jack_devout_get_bpm(AgsSoundcard *soundcard);

static void ags_jack_devout_set_start_note_offset(AgsSoundcard *soundcard,
						  guint64 start_note_offset);
static guint64 ags_jack_devout_get_start_note_offset(AgsSoundcard *soundcard);

static GObject* ags_jack_devout_get_frame_clock(AgsSoundcard *soundcard);

static gboolean ags_jack_devout_is_starting(AgsSoundcard *soundcard);
static gboolean ags_jack_devout_is_playing(AgsSoundcard *soundcard);

static gchar* ags_jack_devout_get_uptime(AgsSoundcard *soundcard);

static void ags_jack_devout_port_init(AgsSoundcard *soundcard,
				      GError **error);
static void ags_jack_devout_port_play(AgsSoundcard *soundcard,
				      GError **error);
static void ags_jack_devout_port_free(AgsSoundcard *soundcard);

static void ags_jack_devout_tic(AgsSoundcard *soundcard);
static void ags_jack_devout_offset_changed(AgsSoundcard *soundcard,
					   guint64 note_offset);

static void* ags_jack_devout_get_buffer(AgsSoundcard *soundcard);
static void* ags_jack_devout_get_next_buffer(AgsSoundcard *soundcard);
static void* ags_jack_devout_get_prev_buffer(AgsSoundcard *soundcard);

static void ags_jack_devout_lock_buffer(AgsSoundcard *soundcard,
					void *buffer);
static void ags_jack_devout_unlock_buffer(AgsSoundcard *soundcard,
					  void *buffer);

static guint ags_jack_devout_get_sub_block_count(AgsSoundcard *soundcard);

static gboolean ags_jack_devout_trylock_sub_block(AgsSoundcard *soundcard,
						  void *buffer, guint sub_block);
static void ags_jack_devout_unlock_sub_block(AgsSoundcard *soundcard,
					     void *buffer, guint sub_block);
...
ags_jack_devout_init(AgsJackDevout *jack_devout)
{
  ...
  /* frame clock */
  jack_devout->frame_clock = ags_frame_clock_new();
			     ...
			     GParamSpec *param_spec)
{
  ...
  case PROP_BUFFER_SIZE:
	...
	return;
      ...
      ags_frame_clock_set_buffer_size(jack_devout->frame_clock,
				      buffer_size);
  ...
  case PROP_SAMPLERATE:
	...
	return;
      ...
      ags_frame_clock_set_samplerate(jack_devout->frame_clock,
				     samplerate);
  ...
  case PROP_BPM:
    {
      ...
      ags_frame_clock_set_bpm(jack_devout->frame_clock,
			      bpm);
...
ags_jack_devout_finalize(GObject *gobject)
{
  ...
  /* frame clock */
  if(jack_devout->frame_clock != NULL){
    g_object_unref(jack_devout->frame_clock);
  }
...
ags_jack_devout_get_uptime(AgsSoundcard *soundcard)
{
  ...
  uptime = ags_frame_clock_to_time_string(jack_devout->frame_clock);
...
ags_jack_devout_port_free(AgsSoundcard *soundcard)
{
  ...
  /*  */
  ags_frame_clock_stop(jack_devout->frame_clock);
...
ags_jack_devout_tic(AgsSoundcard *soundcard)
{
  ...
  ags_frame_clock_increment_counter(jack_devout->frame_clock);

  /* 16th pulse */
  if(ags_frame_clock_get_has_16th_pulse(jack_devout->frame_clock)){
    ags_soundcard_offset_changed(soundcard,
				 jack_devout->frame_clock->note_offset);
  }
...
GObject*
ags_jack_devout_get_frame_clock(AgsSoundcard *soundcard)
{
  AgsJackDevout *jack_devout;
  ...
  frame_clock = (GObject *) jack_devout->frame_clock;
