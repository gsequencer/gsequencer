...
ags_pulse_devout_port_free(AgsSoundcard *soundcard)
{
  ...
  if(pulse_devout->note_256th_delay >= 1.0){
    ...
    for(i = 1; i < (guint) floor(1.0 / (pulse_devout->note_256th_delay)) && note_256th_attack_lower + (guint) floor((double) i * (pulse_devout->note_256th_delay * (double) buffer_size)) < buffer_size; i++){
      if(note_256th_attack_lower + (guint) floor((double) i * (pulse_devout->note_256th_delay * (double) buffer_size)) < note_256th_attack_upper + (guint) floor(pulse_devout->note_256th_delay * (double) buffer_size)){
	pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + i;
      }
    }
  }
...
ags_pulse_devout_tic(AgsSoundcard *soundcard)
{
  ...
  if(delay_counter + 1.0 >= floor(delay)){
	...
	if(note_256th_attack_lower + ((guint) floor(1.0 / note_256th_delay) * (note_256th_delay * buffer_size)) < buffer_size){
	  pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + (guint) floor(1.0 / note_256th_delay);
	}else{
	  pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + (guint) floor(1.0 / note_256th_delay) - 1;
	}
      }
