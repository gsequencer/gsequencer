...
ags_pulse_devout_init(AgsPulseDevout *pulse_devout)
{
  ...
  pulse_devout->note_256th_tic_size = 1.0 / (pulse_devout->delay[0] / 16.0);

  if(pulse_devout->note_256th_tic_size <= 1.0){
    pulse_devout->note_256th_offset_last = 1;
  }else{
    pulse_devout->note_256th_offset_last = pulse_devout->note_256th_tic_size;
  }
}
...
ags_pulse_devout_port_free(AgsSoundcard *soundcard)
{
  ...
  if(pulse_devout->note_256th_tic_size <= 1.0){
    pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + 1;
  }else{
    pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + (guint) fmod(pulse_devout->tic_counter * pulse_devout->delay[pulse_devout->tic_counter] * pulse_devout->note_256th_tic_size, pulse_devout->note_256th_tic_size);
  }
...
ags_pulse_devout_tic(AgsSoundcard *soundcard)
{
  ...
  note_256th_tic_size = pulse_devout->note_256th_tic_size;
  ...
  if(delay_counter + 1.0 >= floor(delay)){
	...
	pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + (guint) fmod(pulse_devout->tic_counter * delay * note_256th_tic_size, note_256th_tic_size);
      }
    ...
    pulse_devout->note_256th_offset = 16 * pulse_devout->note_offset + (pulse_devout->delay_counter * note_256th_tic_size);
    pulse_devout->note_256th_offset_last = pulse_devout->note_256th_offset + (guint) fmod(pulse_devout->tic_counter * delay * note_256th_tic_size, note_256th_tic_size);
				       ...
				       guint *offset_upper)
{
  ...
  if(offset_lower != NULL){
    offset_lower[0] = pulse_devout->note_256th_offset;
  }

  if(offset_upper != NULL){
    offset_upper[0] = pulse_devout->note_256th_offset_last;
  }
