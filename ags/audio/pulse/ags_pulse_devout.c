...
ags_pulse_devout_tic(AgsSoundcard *soundcard)
{
  ...
  if(delay_counter + 1.0 >= floor(delay)){
    if(do_loop &&
      ...
      pulse_devout->note_256th_offset = 16 * loop_left;
	...
	for(; i < (guint) ceil(1.0 / note_256th_delay); i++){
      ...
      pulse_devout->note_256th_offset = 16 * (note_offset + 1);
    ...
    pulse_devout->note_256th_offset = (16 * pulse_devout->note_offset) + (guint) floor((pulse_devout->delay_counter + 1.0) * (1.0 / note_256th_delay));
