...
{
  static GType ags_type_route_lv2_audio = 0;

  if(!ags_type_route_lv2_audio){
  ...
  return(ags_type_route_lv2_audio);
}
...
ags_route_lv2_audio_class_init(AgsRouteLv2AudioClass *route_lv2_audio)
{
  ...
  param_spec = g_param_spec_object("notation-input\0",
				   ...
				   "Route notation as input to the LV2 recall.\0",
				   AGS_TYPE_PORT,
  ...
  param_spec = g_param_spec_object("sequencer-input\0",
				   ...
				   "Route sequencer as input to the LV2 recall.\0",
				   AGS_TYPE_PORT,
				   G_PARAM_READABLE | G_PARAM_WRITABLE);
  g_object_class_install_property(gobject,
				  PROP_SEQUENCER_INPUT,
...
ags_route_lv2_audio_init(AgsRouteLv2Audio *route_lv2_audio)
{
  ...
  AGS_RECALL(route_lv2_audio)->name = "ags-count-beats\0";
 ...
 *
 * Returns: a new #AgsRouteLv2Audio
 *
 * Since: 0.7.1
 */
AgsRouteLv2Audio*
