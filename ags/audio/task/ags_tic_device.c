...
ags_tic_device_launch(AgsTask *task)
{
  ...
  if(AGS_IS_SOUNDCARD(tic_device->device)){
    ...
    if((soundcard_interface->is_playing != NULL && ags_soundcard_is_playing(AGS_SOUNDCARD(tic_device->device))) ||
       (soundcard_interface->is_recording != NULL && ags_soundcard_is_recording(AGS_SOUNDCARD(tic_device->device)))){
    ...
    if((sequencer_interface->is_playing != NULL && ags_sequencer_is_playing(AGS_SEQUENCER(tic_device->device))) ||
       (sequencer_interface->is_recording != NULL && ags_sequencer_is_recording(AGS_SEQUENCER(tic_device->device)))){
