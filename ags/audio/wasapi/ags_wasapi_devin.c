...
ags_wasapi_devin_init(AgsWasapiDevin *wasapi_devin)
{
  ...
  if(wasapi_devin->note_256th_delay >= 1.0){
    wasapi_devin->note_256th_offset_last = 0;
  }else{
    wasapi_devin->note_256th_offset_last = (guint) floor(1.0 / wasapi_devin->note_256th_delay);
  }
}
...
ags_wasapi_devin_tic(AgsSoundcard *soundcard)
{
  ...
  if((16 * (note_offset + 1) >= next_note_256th_offset_lower &&
	...
	for(; i < (guint) ceil(1.0 / note_256th_delay); i++){
      ...
      wasapi_devin->note_256th_attack_of_16th_pulse = note_256th_attack_of_16th_pulse;      
