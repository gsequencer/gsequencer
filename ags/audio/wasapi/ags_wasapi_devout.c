...
static const GUID CLSID_MMDeviceEnumerator = __uidof(MMDeviceEnumerator); // {0xBCDE0395, 0xE52F, 0x467C, 0x8E, 0x3D, 0xC4, 0x57, 0x92, 0x91, 0x69, 0x2E};
static const GUID IID_IMMDeviceEnumerator = __uidof(IMMDeviceEnumerator); // {0xA95664D2, 0x9614, 0x4F35, 0xA7, 0x46, 0xDE, 0x8D, 0xB6, 0x36, 0x17, 0xE6};
static const GUID IID_IAudioClient = __uidof(IAudioClient); //{0x1CB9AD4C, 0xDBFA, 0x4c32, 0xB1, 0x78, 0xC2, 0xF5, 0x68, 0xA7, 0x03, 0xB2};
static const GUID IID_IAudioRenderClient = __uidof(IAudioRenderClient); // {0xF294ACFC, 0x3146, 0x4483, 0xA7, 0xBF, 0xAD, 0xDC, 0xA7, 0xC2, 0x60, 0xE2};
...
ags_wasapi_devout_init(AgsWasapiDevout *wasapi_devout)
{
  ...
  g_atomic_int_set(&(wasapi_devout->sync_flags),
		   AGS_WASAPI_DEVOUT_PASS_THROUGH);
  ...
  /* buffer */
  wasapi_devout->buffer_mutex = (pthread_mutex_t **) malloc(8 * sizeof(pthread_mutex_t *));

  for(i = 0; i < 8; i++){
    wasapi_devout->buffer_mutex[i] = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));

    pthread_mutex_init(wasapi_devout->buffer_mutex[i],
		       NULL);
  }

  wasapi_devout->sub_block_count = AGS_SOUNDCARD_DEFAULT_SUB_BLOCK_COUNT;
  wasapi_devout->sub_block_mutex = (pthread_mutex_t **) malloc(8 * wasapi_devout->sub_block_count * wasapi_devout->pcm_channels * sizeof(pthread_mutex_t *));
  ...
  /* callback mutex */
  wasapi_devout->callback_mutex = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));
  pthread_mutex_init(wasapi_devout->callback_mutex,
		     NULL);

  wasapi_devout->callback_cond = (pthread_cond_t *) malloc(sizeof(pthread_cond_t));
  pthread_cond_init(wasapi_devout->callback_cond, NULL);

  /* callback finish mutex */
  wasapi_devout->callback_finish_mutex = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));
  pthread_mutex_init(wasapi_devout->callback_finish_mutex,
		     NULL);

  wasapi_devout->callback_finish_cond = (pthread_cond_t *) malloc(sizeof(pthread_cond_t));
  pthread_cond_init(wasapi_devout->callback_finish_cond, NULL);

  /*  */
  wasapi_devout->notify_soundcard = NULL;
}
  ...
  case PROP_DEVICE:
    {
      ...
      wasapi_devout->card_uri = g_strdup(device);
  ...
  case PROP_DEVICE:
    {
      ...
      g_value_set_string(value, wasapi_devout->card_uri);
...
ags_wasapi_devout_dispose(GObject *gobject)
{
  ...
  /* notify soundcard */
  if(wasapi_devout->notify_soundcard != NULL){
    ...
    ags_task_thread_remove_cyclic_task(task_thread,
				       (AgsTask *) wasapi_devout->notify_soundcard);
    
    g_object_unref(wasapi_devout->notify_soundcard);
...
ags_wasapi_devout_finalize(GObject *gobject)
{
  ...
  pthread_mutex_destroy(wasapi_devout->obj_mutex);
  free(wasapi_devout->obj_mutex);

  pthread_mutexattr_destroy(wasapi_devout->obj_mutexattr);
  free(wasapi_devout->obj_mutexattr);
...
ags_wasapi_devout_is_ready(AgsConnectable *connectable)
{
  ...
  is_ready = (((AGS_WASAPI_DEVOUT_ADDED_TO_REGISTRY & (wasapi_devout->flags)) != 0) ? TRUE: FALSE);
...
ags_wasapi_devout_is_connected(AgsConnectable *connectable)
{
  ...
  is_connected = (((AGS_WASAPI_DEVOUT_CONNECTED & (wasapi_devout->flags)) != 0) ? TRUE: FALSE);
...
/**
 * ags_wasapi_devout_get_class_mutex:
 * 
...
ags_wasapi_devout_get_class_mutex()
{
  return(&ags_wasapi_devout_class_mutex);
}
...
ags_wasapi_devout_get_device(AgsSoundcard *soundcard)
{
  ...
  device = g_strdup(wasapi_devout->card_uri);
			     ...
			     GList **card_id, GList **card_name)
{
  ...
  if(!CoCreateInstance(&CLSID_MMDeviceEnumerator, 0, CLSCTX_ALL, &IID_IMMDeviceEnumerator, &dev_enumrator)){
			    ...
			    GError **error)
{
  ...
  if(dev_enumerator->lpVtbl->GetDevice(dev_enumerator, DevID, &mm_device)){
    if(error != NULL){
      g_set_error(error,
		  AGS_WASAPI_DEVOUT_ERROR,
		  AGS_WASAPI_DEVOUT_ERROR_LOCKED_SOUNDCARD,
    ...
    dev_numerator->lpVtbl->Release(dev_enumerator);
    CoUninitialize();
  ...
  desired_format.Format.nBlockAlign = 2 * (BitResolution/8);
  ...
  wasapi_devout->nth_ring_buffer = 0;
			      ...
			      GError **error)
{
  ...
  notify_soundcard = AGS_NOTIFY_SOUNDCARD(wasapi_devout->notify_soundcard);
  ...
  audio_client->lpVtbl->GetService(audio_client, &IID_IAudioRenderClient, (void **) &audio_render_client);
  ...
  g_atomic_int_set(&(wasapi_devout->available),
		   FALSE);
  
  if(wasapi_devout->nth_ring_buffer + 1 >= wasapi_devout->ring_buffer_size){
    wasapi_devout->nth_ring_buffer = 0;
  }else{
    wasapi_devout->nth_ring_buffer += 1;
  }
