...
/**
 * ags_math_util_find_function_all:
 * @str: the string
 * 
 * Find all functions.
 * 
 * Returns: the string vector containing the functions
 * 
 * Since: 3.6.0
 */
gchar**
ags_math_util_find_function_all(gchar *str)
{
  gchar **function_arr;
...
/**
 * ags_math_util_find_coefficient:
 * @str: the string
 * 
 * Find next coefficient.
 * 
 * Returns: the string offset matching coefficient, otherwise %NULL
 * 
 * Since: 3.6.0
 */
gchar*
ags_math_util_find_coefficient(gchar *str)
{
  ...
  g_regex_match(literal_regex, str, 0, &literal_match_info);
 
  while(g_match_info_matches(literal_match_info)){
    gint literal_start_pos, literal_end_pos;
    ...
    g_match_info_fetch_pos(literal_match_info,
			   0,
			   &literal_start_pos, &literal_end_pos);

    next = literal_start_pos;
    
    if(prev == -1){
      tmp_str = g_strdup_printf("%*.s",
				literal_start_pos, str);
    }else{
      tmp_str = g_strdup_printf("%*.s",
				(literal_start_pos - prev), str + prev);
    }
    
    if(tmp_str != NULL){
      g_regex_match(coefficient_regex, tmp_str, 0, &coefficient_match_info);

      if(g_match_info_matches(coefficient_match_info)){
	gint coefficient_start_pos, coefficient_end_pos;

	g_match_info_fetch_pos(coefficient_match_info,
			       0,
			       &coefficient_start_pos, &coefficient_end_pos);

	if(prev != -1){
	  retval = str + prev + coefficient_start_pos;
	}else{
	  retval = str + coefficient_start_pos;
	}
      }
...
/**
 * ags_math_util_find_coefficient_all:
 * @str: the string
 * 
 * Find all coefficients.
 * 
 * Returns: the string vector containing the coefficients
 * 
 * Since: 3.6.0
 */
gchar**
ags_math_util_find_coefficient_all(gchar *str)
{
  GMatchInfo *literal_match_info;
  GMatchInfo *coefficient_match_info;
  ...
  while(g_match_info_matches(literal_match_info)){
    ...
    if(prev == -1){
      if(start_pos != 0){
	if(next == -1){
	  current_coefficient = g_strndup(str,
				      start_pos);
	}else{
	  if(start_pos - next > 0){
	    current_coefficient = g_strndup(str + next,
					start_pos - next);
	  }
	}
    ...
    if(current_coefficient != NULL){
      g_regex_match(coefficient_regex, current_coefficient, 0, &coefficient_match_info);
    
      while(g_match_info_matches(coefficient_match_info)){
	gchar *tmp_coefficient;

	tmp_coefficient = g_match_info_fetch(coefficient_match_info,
					 0);
	
	if(coefficient_arr == NULL){
	  coefficient_arr = (gchar **) g_malloc(2 * sizeof(gchar *));

	  coefficient_arr[0] = g_strdup(tmp_coefficient);
	  coefficient_arr[1] = NULL;

#ifdef AGS_DEBUG	    
	  g_message("found %s", coefficient_arr[0]);
#endif
	  
	  n_coefficients++;
	}else{
	  if(!g_strv_contains(coefficient_arr,
			      tmp_coefficient)){
	    coefficient_arr = (gchar **) g_realloc(coefficient_arr,
					       (n_coefficients + 2) * sizeof(gchar *));

	    coefficient_arr[n_coefficients] = g_strdup(tmp_coefficient);
	    coefficient_arr[n_coefficients + 1] = NULL;

#ifdef AGS_DEBUG	    
	    g_message("found %s", coefficient_arr[n_coefficients]);
#endif
  ...
  if(prev == -1){
    if(next == -1){
      current_coefficient = g_strdup(str);
    }else{
...
/**
 * ags_math_util_find_exponent:
 * @str: the string
 * 
 * Find next exponent.
 * 
 * Returns: the string offset matching exponent, otherwise %NULL
 * 
 * Since: 3.6.0
 */
gchar*
ags_math_util_find_exponent(gchar *str)
{
  GMatchInfo *function_match_info;
  GMatchInfo *quantity_match_info;
  ...
  static const GRegex *function_regex = NULL;
  static const GRegex *quantity_regex = NULL;
  static const GRegex *exponent_regex = NULL;

  static const gchar *function_pattern = "(log|exp|sin|cos|tan|asin|acos|atan|floor|ceil|round)";

  /* groups: #1 symbol, #2 numeric base and fraction */
  static const gchar *quantity_pattern = "([a-zA-Z][0-9]*)|([ℯ𝜋𝑖∞]+)|([0-9]+(\\.[0-9]+)?)";
  ...
  if(quantity_regex == NULL){
    error = NULL;
    quantity_regex = g_regex_new(quantity_pattern,
				 (G_REGEX_EXTENDED),
				 0,
				 &error);
  ...
  while(g_match_info_matches(function_match_info)){
    ...
    if(tmp_str != NULL){
      g_regex_match(quantity_regex, tmp_str, 0, &quantity_match_info);

      if(g_match_info_matches(quantity_match_info)){
	gint quantity_start_pos, quantity_end_pos;

	g_match_info_fetch_pos(quantity_match_info,
			       0,
			       &quantity_start_pos, &quantity_end_pos);


	if(prev != -1){
	  g_regex_match(exponent_regex, str + prev + quantity_end_pos, 0, &exponent_match_info);
	}else{
	  g_regex_match(exponent_regex, str + quantity_end_pos, 0, &exponent_match_info);
	}
	
	if(g_match_info_matches(exponent_match_info)){
	  gint exponent_start_pos, exponent_end_pos;

	  g_match_info_fetch_pos(exponent_match_info,
				 0,
				 &exponent_start_pos, &exponent_end_pos);

	  if(prev != -1){
	    retval = str + prev + exponent_start_pos;
	  }else{
	    retval = str + exponent_start_pos;
	  }
	}
	...
	g_match_info_free(exponent_match_info);
      ...
      g_match_info_free(quantity_match_info);
  ...
  if(retval == NULL){
	...
	if(prev != -1){
	  retval = str + prev + quantity_start_pos;
	}else{
	  retval = str + quantity_start_pos;
	}
      }
...
/**
 * ags_math_util_find_exponent_all:
 * @str: the string
 * 
 * Find all exponents.
 * 
 * Returns: the string vector containing the exponents
 * 
 * Since: 3.6.0
 */
gchar**
ags_math_util_find_exponent_all(gchar *str)
{
  GMatchInfo *function_match_info;
  GMatchInfo *quantity_match_info;
  GMatchInfo *exponent_match_info;
  ...
  while(g_match_info_matches(function_match_info)){
    ...
    if(prev == -1){
      if(start_pos != 0){
	if(next == -1){
	  current_quantity = g_strndup(str,
				       start_pos);
	}else{
	  if(start_pos - next > 0){
	    current_quantity = g_strndup(str + next,
					 start_pos - next);
	  }
	}
    ...
    if(current_quantity != NULL){
      g_regex_match(quantity_regex, current_quantity, 0, &quantity_match_info);
    
      while(g_match_info_matches(quantity_match_info)){
	gint quantity_start_pos, quantity_end_pos;
	...
	g_regex_match(exponent_regex, current_quantity + quantity_end_pos, 0, &exponent_match_info);
	
	if(g_match_info_matches(exponent_match_info)){
	  gchar *tmp_exponent;

	  tmp_exponent = g_match_info_fetch(exponent_match_info,
					    0);
	
	  if(exponent_arr == NULL){
	    exponent_arr = (gchar **) g_malloc(2 * sizeof(gchar *));

	    exponent_arr[0] = g_strdup(tmp_exponent);
	    exponent_arr[1] = NULL;

#ifdef AGS_DEBUG	    
	    g_message("found %s", exponent_arr[0]);
#endif
	  
	    n_exponents++;
	  }else{
	    if(!g_strv_contains(exponent_arr,
				tmp_exponent)){
	      exponent_arr = (gchar **) g_realloc(exponent_arr,
						  (n_exponents + 2) * sizeof(gchar *));

	      exponent_arr[n_exponents] = g_strdup(tmp_exponent);
	      exponent_arr[n_exponents + 1] = NULL;

#ifdef AGS_DEBUG	    
	      g_message("found %s", exponent_arr[n_exponents]);
#endif
	...
	g_match_info_next(quantity_match_info,
			  NULL);
      }
