...
/**
 * ags_vst3_conversion_test_flags:
 * @vst3_conversion: the #AgsVst3Conversion
 * @flags: the flags
 * 
 ...
 */
gboolean
ags_vst3_conversion_test_flags(AgsVst3Conversion *vst3_conversion, guint flags)
{
  gboolean retval;
  
  GRecMutex *conversion_mutex;

  if(!AGS_IS_VST3_CONVERSION(vst3_conversion)){
    return(FALSE);
  }
  ...
  retval = ((flags & (vst3_conversion->flags)) != 0) ? TRUE: FALSE;
 ...
 */
void
ags_vst3_conversion_set_flags(AgsVst3Conversion *vst3_conversion, guint flags)
{
  GRecMutex *conversion_mutex;
  ...
  vst3_conversion->flags |= flags;
  
  g_rec_mutex_unlock(conversion_mutex);
}

/**
 * ags_vst3_conversion_unset_flags:
 * @vst3_conversion: the #AgsVst3Conversion
 * @flags: the flags
 *
 * Unset flags.
 ...
 */
void
ags_vst3_conversion_unset_flags(AgsVst3Conversion *vst3_conversion, guint flags)
{
  GRecMutex *conversion_mutex;
