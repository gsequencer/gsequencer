...
int ags_functional_loop_001_xml_test_init_suite();
int ags_functional_loop_001_xml_test_clean_suite();
...
#define AGS_FUNCTIONAL_LOOP_001_XML_TEST_FILE_SETUP_FILENAME AGS_SRC_DIR "/" "ags/test/X/examples/ags-loop-000.xml"
...
void
ags_functional_loop_001_xml_test_add_test()
{
  /* add the tests to the suite */
  if((CU_add_test(pSuite, "functional test of GSequencer setup by file and play it", ags_functional_loop_001_xml_test_file_setup) == NULL)){
    CU_cleanup_registry();
 ...
 */
int
ags_functional_loop_001_xml_test_clean_suite()
{  
  return(0);
}

void
ags_functional_loop_001_xml_test_file_setup()
{
  ...
  for(i = 0; success && i < AGS_FUNCTIONAL_LOOP_001_XML_TEST_FILE_SETUP_PLAYBACK_COUNT; i++){
    ...
    while(!expired){  
      ...
      if(start_time.tv_sec + AGS_FUNCTIONAL_LOOP_001_XML_TEST_FILE_SETUP_PLAYBACK_DURATION < current_time.tv_sec){
	expired = TRUE;
      }
    }
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsFunctionalLoop001XmlTest", ags_functional_loop_001_xml_test_init_suite, ags_functional_loop_001_xml_test_clean_suite);
