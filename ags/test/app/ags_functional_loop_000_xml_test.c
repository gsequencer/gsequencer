...
void ags_functional_loop_000_xml_test_add_test();

int ags_functional_loop_000_xml_test_init_suite();
int ags_functional_loop_000_xml_test_clean_suite();

void ags_functional_loop_000_xml_test_file_setup();

#define AGS_FUNCTIONAL_LOOP_000_XML_TEST_DEFAULT_IDLE_TIME (3.0 * G_USEC_PER_SEC)

#define AGS_FUNCTIONAL_LOOP_000_XML_TEST_FILE_SETUP_PLAYBACK_COUNT (1)
#define AGS_FUNCTIONAL_LOOP_000_XML_TEST_FILE_SETUP_PLAYBACK_DURATION (120)

#define AGS_FUNCTIONAL_LOOP_000_XML_TEST_FILE_SETUP_FILENAME AGS_SRC_DIR "/" "ags/test/app/examples/ags-loop-000.xml"

#define AGS_FUNCTIONAL_LOOP_000_XML_TEST_CONFIG "[generic]\n" \
...
ags_functional_loop_000_xml_test_add_test()
{
  ...
  if((CU_add_test(pSuite, "functional test of GSequencer setup by file and play it", ags_functional_loop_000_xml_test_file_setup) == NULL)){
    CU_cleanup_registry();
 ...
 */
int
ags_functional_loop_000_xml_test_init_suite()
{
  ...
  ags_functional_test_util_idle(AGS_FUNCTIONAL_LOOP_000_XML_TEST_DEFAULT_IDLE_TIME);
 ...
 */
int
ags_functional_loop_000_xml_test_clean_suite()
{  
  return(0);
}

void
ags_functional_loop_000_xml_test_file_setup()
{
  ...
  for(i = 0; success && i < AGS_FUNCTIONAL_LOOP_000_XML_TEST_FILE_SETUP_PLAYBACK_COUNT; i++){
    ...
    while(!expired){  
      ...
      if(start_time.tv_sec + AGS_FUNCTIONAL_LOOP_000_XML_TEST_FILE_SETUP_PLAYBACK_DURATION < current_time.tv_sec){
	expired = TRUE;
      }
    }
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsFunctionalLoop000XmlTest", ags_functional_loop_000_xml_test_init_suite, ags_functional_loop_000_xml_test_clean_suite);
