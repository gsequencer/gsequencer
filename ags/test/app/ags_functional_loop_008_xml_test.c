...
void ags_functional_loop_008_xml_test_add_test();

int ags_functional_loop_008_xml_test_init_suite();
int ags_functional_loop_008_xml_test_clean_suite();

void ags_functional_loop_008_xml_test_file_setup();

#define AGS_FUNCTIONAL_LOOP_008_XML_TEST_DEFAULT_IDLE_TIME (3.0 * G_USEC_PER_SEC)
...
#define AGS_FUNCTIONAL_LOOP_008_XML_TEST_FILE_SETUP_FILENAME AGS_SRC_DIR "/" "ags/test/app/examples/ags-loop-008.xml"
...
struct timespec ags_functional_loop_008_xml_test_default_timeout = {
  300,
  0,
};

void
ags_functional_loop_008_xml_test_add_test()
{
  /* add the tests to the suite */
  if((CU_add_test(pSuite, "functional test of GSequencer setup by file and play it", ags_functional_loop_008_xml_test_file_setup) == NULL)){
    CU_cleanup_registry();
 ...
 */
int
ags_functional_loop_008_xml_test_init_suite()
{
  ...
  ags_functional_test_util_idle_condition_and_timeout(AGS_FUNCTIONAL_TEST_UTIL_IDLE_CONDITION(ags_functional_test_util_idle_test_widget_realized),
						      &ags_functional_loop_008_xml_test_default_timeout,
  ...
  ags_functional_test_util_idle(AGS_FUNCTIONAL_LOOP_008_XML_TEST_DEFAULT_IDLE_TIME);
 ...
 */
int
ags_functional_loop_008_xml_test_clean_suite()
{  
  return(0);
}

void
ags_functional_loop_008_xml_test_file_setup()
{
  ...
  for(i = 0; success && i < AGS_FUNCTIONAL_LOOP_008_XML_TEST_FILE_SETUP_PLAYBACK_COUNT; i++){
    ...
    while(!expired){  
      ...
      if(start_time.tv_sec + AGS_FUNCTIONAL_LOOP_008_XML_TEST_FILE_SETUP_PLAYBACK_DURATION < current_time.tv_sec){
	expired = TRUE;
      }
    }
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsFunctionalLoop008XmlTest", ags_functional_loop_008_xml_test_init_suite, ags_functional_loop_008_xml_test_clean_suite);
  ...
  new_argv[argc + 1] = AGS_FUNCTIONAL_LOOP_008_XML_TEST_FILE_SETUP_FILENAME;
  ...
  if((str = getenv("AGS_TEST_CONFIG")) != NULL){
    ...
    ags_functional_test_util_init(&argc, &new_argv,
				  AGS_FUNCTIONAL_LOOP_008_XML_TEST_CONFIG);
  }
#endif
  
  ags_functional_test_util_do_run(argc, new_argv,
				  ags_functional_loop_008_xml_test_add_test, &is_available);
