...
void ags_functional_syncsynth_test_add_test();

int ags_functional_syncsynth_test_init_suite();
int ags_functional_syncsynth_test_clean_suite();

void ags_functional_syncsynth_test_resize_pads();
void ags_functional_syncsynth_test_resize_audio_channels();

#define AGS_FUNCTIONAL_SYNCSYNTH_TEST_DEFAULT_IDLE_TIME (3.0 * G_USEC_PER_SEC)

#define AGS_FUNCTIONAL_SYNCSYNTH_TEST_RESIZE_OUTPUT_PADS (5)
#define AGS_FUNCTIONAL_SYNCSYNTH_TEST_RESIZE_INPUT_PADS (15)

#define AGS_FUNCTIONAL_SYNCSYNTH_TEST_RESIZE_AUDIO_CHANNELS (0)

#define AGS_FUNCTIONAL_SYNCSYNTH_TEST_CONFIG "[generic]\n"	\
...
struct timespec ags_functional_syncsynth_test_default_timeout = {
  300,
  0,
};

void
ags_functional_syncsynth_test_add_test()
{
  /* add the tests to the suite */
  if((CU_add_test(pSuite, "functional test of AgsSyncsynth resize pads", ags_functional_syncsynth_test_resize_pads) == NULL) ||
     (CU_add_test(pSuite, "functional test of AgsSyncsynth resize audio channels", ags_functional_syncsynth_test_resize_audio_channels) == NULL)){
    CU_cleanup_registry();
 ...
 */
int
ags_functional_syncsynth_test_init_suite()
{
  ...
  ags_functional_test_util_idle_condition_and_timeout(AGS_FUNCTIONAL_TEST_UTIL_IDLE_CONDITION(ags_functional_test_util_idle_test_widget_realized),
						      &ags_functional_syncsynth_test_default_timeout,
 ...
 */
int
ags_functional_syncsynth_test_clean_suite()
{  
  return(0);
}
...
ags_functional_syncsynth_test_resize_pads()
{
  ...
  syncsynth = g_list_nth_data(start_list,
			      nth_machine);
  ...
  ags_functional_test_util_idle_condition_and_timeout(AGS_FUNCTIONAL_TEST_UTIL_IDLE_CONDITION(ags_functional_test_util_idle_test_widget_visible),
						      &ags_functional_syncsynth_test_default_timeout,
						      &(AGS_MACHINE(syncsynth)->machine_editor_dialog));

  ags_functional_test_util_sync();
  
  ags_functional_test_util_idle_condition_and_timeout(AGS_FUNCTIONAL_TEST_UTIL_IDLE_CONDITION(ags_functional_test_util_idle_test_widget_visible),
						      &ags_functional_syncsynth_test_default_timeout,
						      &(AGS_MACHINE_EDITOR_DIALOG(AGS_MACHINE(syncsynth)->machine_editor_dialog)->machine_editor));

  ags_functional_test_util_sync();

  ags_functional_test_util_idle_condition_and_timeout(AGS_FUNCTIONAL_TEST_UTIL_IDLE_CONDITION(ags_functional_test_util_idle_test_widget_visible),
						      &ags_functional_syncsynth_test_default_timeout,
						      &(AGS_MACHINE_EDITOR_DIALOG(AGS_MACHINE(syncsynth)->machine_editor_dialog)->machine_editor->resize_editor));
  ...
  ags_functional_test_util_machine_editor_dialog_resize_outputs(nth_machine,
								AGS_FUNCTIONAL_SYNCSYNTH_TEST_RESIZE_OUTPUT_PADS);
  ...
  ags_functional_test_util_machine_editor_dialog_resize_inputs(nth_machine,
							       AGS_FUNCTIONAL_SYNCSYNTH_TEST_RESIZE_INPUT_PADS);
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsFunctionalSyncsynthTest", ags_functional_syncsynth_test_init_suite, ags_functional_syncsynth_test_clean_suite);
  ...
  if((str = getenv("AGS_TEST_CONFIG")) != NULL){
    ...
    ags_functional_test_util_init(&argc, &argv,
				  AGS_FUNCTIONAL_SYNCSYNTH_TEST_CONFIG);
  }
#endif
  
  ags_functional_test_util_do_run(argc, argv,
				  ags_functional_syncsynth_test_add_test, &is_available);
