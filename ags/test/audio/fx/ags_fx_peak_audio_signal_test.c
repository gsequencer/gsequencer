...
int ags_fx_peak_audio_signal_test_init_suite();
int ags_fx_peak_audio_signal_test_clean_suite();

void ags_fx_peak_audio_signal_test_new();
void ags_fx_peak_audio_signal_test_run_inter();
 ...
 */
int
ags_fx_peak_audio_signal_test_init_suite()
{
 ...
 */
int
ags_fx_peak_audio_signal_test_clean_suite()
{  
  return(0);
}

void
ags_fx_peak_audio_signal_test_new()
{
  ...
  fx_peak_audio_signal = ags_fx_peak_audio_signal_new(audio_signal);

  CU_ASSERT(fx_peak_audio_signal != NULL);
  CU_ASSERT(AGS_RECALL_AUDIO_SIGNAL(fx_peak_audio_signal)->source == audio_signal);
}

void
ags_fx_peak_audio_signal_test_run_inter()
{
  AgsAudio *audio;
  AgsChannel *channel;
  ...
  fx_peak_recycling = ags_fx_peak_recycling_new(recycling);
  ags_recall_set_sound_scope(fx_peak_recycling, AGS_SOUND_SCOPE_PLAYBACK);

  ags_recall_add_child(fx_peak_channel_processor,
		       fx_peak_recycling);
  ...
  fx_peak_audio_signal = ags_fx_peak_audio_signal_new(audio_signal);
  ags_recall_set_sound_scope(fx_peak_audio_signal, AGS_SOUND_SCOPE_PLAYBACK);

  ags_recall_add_child(fx_peak_recycling,
		       fx_peak_audio_signal);

  CU_ASSERT(fx_peak_audio_signal != NULL);

  /* run inter - attempt #0 */  
  ags_recall_run_inter(fx_peak_audio_signal);
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsFxPeakAudioSignalTest", ags_fx_peak_audio_signal_test_init_suite, ags_fx_peak_audio_signal_test_clean_suite);
  ...
  if((CU_add_test(pSuite, "test of AgsFxPeakAudioSignal new", ags_fx_peak_audio_signal_test_new) == NULL) ||
     (CU_add_test(pSuite, "test of AgsFxPeakAudioSignal run inter", ags_fx_peak_audio_signal_test_run_inter) == NULL)){
    CU_cleanup_registry();
