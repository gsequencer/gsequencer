...
int ags_buffer_audio_signal_test_init_suite();
int ags_buffer_audio_signal_test_clean_suite();

void ags_buffer_audio_signal_test_run_inter();

void ags_buffer_audio_signal_test_run_inter_callback(AgsRecall *recall, gpointer data);

#define AGS_BUFFER_AUDIO_SIGNAL_TEST_RUN_INTER_AUDIO_SIGNAL_LENGTH (24)
 ...
 */
int
ags_buffer_audio_signal_test_clean_suite()
{
...
void
ags_buffer_audio_signal_test_run_inter_callback(AgsRecall *recall, gpointer data)
{
  test_run_inter_invoke_count += 1;
}

void
ags_buffer_audio_signal_test_run_inter()
{
  AgsChannel *output;
  AgsChannel *input;
  ...
  ags_audio_signal_stream_resize(audio_signal,
				 AGS_BUFFER_AUDIO_SIGNAL_TEST_RUN_INTER_AUDIO_SIGNAL_LENGTH);
  ...
  CU_ASSERT(AGS_IS_BUFFER_AUDIO_SIGNAL(recall_audio_signal->data));
  CU_ASSERT(AGS_RECALL_CLASS(AGS_BUFFER_AUDIO_SIGNAL_GET_CLASS(recall_audio_signal->data))->run_inter != NULL);

  g_signal_connect(recall_audio_signal->data, "run-inter",
		   G_CALLBACK(ags_buffer_audio_signal_test_run_inter_callback), NULL);

  for(i = 0; i < AGS_BUFFER_AUDIO_SIGNAL_TEST_RUN_INTER_AUDIO_SIGNAL_LENGTH; i++){
  ...
  CU_ASSERT(test_run_inter_invoke_count == AGS_BUFFER_AUDIO_SIGNAL_TEST_RUN_INTER_AUDIO_SIGNAL_LENGTH);
}
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsBufferAudioSignalTest", ags_buffer_audio_signal_test_init_suite, ags_buffer_audio_signal_test_clean_suite);
  ...
  if((CU_add_test(pSuite, "test of AgsBufferAudioSignal run inter", ags_buffer_audio_signal_test_run_inter) == NULL)){
    CU_cleanup_registry();
