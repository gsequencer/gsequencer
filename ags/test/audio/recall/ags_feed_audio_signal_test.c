...
int ags_feed_audio_signal_test_init_suite();
int ags_feed_audio_signal_test_clean_suite();

void ags_feed_audio_signal_test_run_pre();

void ags_feed_audio_signal_test_run_pre_callback(AgsRecall *recall, gpointer data);

#define AGS_FEED_AUDIO_SIGNAL_TEST_RUN_PRE_AUDIO_SIGNAL_LENGTH (24)
 ...
 */
int
ags_feed_audio_signal_test_init_suite()
{
 ...
 */
int
ags_feed_audio_signal_test_clean_suite()
{
...
void
ags_feed_audio_signal_test_run_pre_callback(AgsRecall *recall, gpointer data)
{
  test_run_pre_invoke_count += 1;
}

void
ags_feed_audio_signal_test_run_pre()
{
  AgsChannel *output;
  AgsChannel *input;
  ...
  note = ags_note_new();
  note->x[0] = 0;
  note->x[1] = 64; //NOTE:JK: should match AGS_FEED_AUDIO_SIGNAL_TEST_RUN_PRE_AUDIO_SIGNAL_LENGTH iterations
  note->y = 0;
  ...
  ags_audio_signal_stream_resize(audio_signal,
				 AGS_FEED_AUDIO_SIGNAL_TEST_RUN_PRE_AUDIO_SIGNAL_LENGTH);
  ...
  CU_ASSERT(AGS_IS_FEED_AUDIO_SIGNAL(recall_audio_signal->data));
  CU_ASSERT(AGS_RECALL_CLASS(AGS_FEED_AUDIO_SIGNAL_GET_CLASS(recall_audio_signal->data))->run_pre != NULL);

  g_signal_connect(recall_audio_signal->data, "run-pre",
		   G_CALLBACK(ags_feed_audio_signal_test_run_pre_callback), NULL);

  for(i = 0; i < AGS_FEED_AUDIO_SIGNAL_TEST_RUN_PRE_AUDIO_SIGNAL_LENGTH; i++){
  ...
  CU_ASSERT(test_run_pre_invoke_count == AGS_FEED_AUDIO_SIGNAL_TEST_RUN_PRE_AUDIO_SIGNAL_LENGTH);
}
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsFeedAudioSignalTest", ags_feed_audio_signal_test_init_suite, ags_feed_audio_signal_test_clean_suite);
  ...
  if((CU_add_test(pSuite, "test of AgsFeedAudioSignal run-pre", ags_feed_audio_signal_test_run_pre) == NULL)){
    CU_cleanup_registry();
