...
int ags_stream_audio_signal_test_init_suite();
int ags_stream_audio_signal_test_clean_suite();

void ags_stream_audio_signal_test_run_post();

void ags_stream_audio_signal_test_run_post_callback(AgsRecall *recall, gpointer data);

#define AGS_STREAM_AUDIO_SIGNAL_TEST_RUN_POST_AUDIO_SIGNAL_LENGTH (24)
...
guint test_run_post_invoke_count = 0;
 ...
 */
int
ags_stream_audio_signal_test_init_suite()
{
 ...
 */
int
ags_stream_audio_signal_test_clean_suite()
{
...
void
ags_stream_audio_signal_test_run_post_callback(AgsRecall *recall, gpointer data)
{
  test_run_post_invoke_count += 1;
}

void
ags_stream_audio_signal_test_run_post()
{
  AgsChannel *output;
  AgsChannel *input;
  ...
  ags_audio_signal_stream_resize(audio_signal,
				 AGS_STREAM_AUDIO_SIGNAL_TEST_RUN_POST_AUDIO_SIGNAL_LENGTH);
  ...
  CU_ASSERT(AGS_IS_STREAM_AUDIO_SIGNAL(recall_audio_signal->data));
  CU_ASSERT(AGS_RECALL_CLASS(AGS_STREAM_AUDIO_SIGNAL_GET_CLASS(recall_audio_signal->data))->run_post != NULL);

  g_signal_connect(recall_audio_signal->data, "run-post",
		   G_CALLBACK(ags_stream_audio_signal_test_run_post_callback), NULL);

  for(i = 0; i < AGS_STREAM_AUDIO_SIGNAL_TEST_RUN_POST_AUDIO_SIGNAL_LENGTH; i++){
    ags_recall_run_post(recall_audio_signal->data);
  }

  CU_ASSERT(test_run_post_invoke_count == AGS_STREAM_AUDIO_SIGNAL_TEST_RUN_POST_AUDIO_SIGNAL_LENGTH);
}
...
main(int argc, char **argv)
{
  ...
  pSuite = CU_add_suite("AgsStreamAudioSignalTest", ags_stream_audio_signal_test_init_suite, ags_stream_audio_signal_test_clean_suite);
  ...
  if((CU_add_test(pSuite, "test of AgsStreamAudioSignal run inter", ags_stream_audio_signal_test_run_post) == NULL)){
    CU_cleanup_registry();
