...
ags_thread_real_clock(AgsThread *thread)
{
  ...
  void ags_thread_clock_sync(AgsThread *thread){
	...
	thread->tic_delay = AGS_THREAD(g_atomic_pointer_get(&(thread->parent)))->tic_delay;
      }
  ...
  if(g_atomic_pointer_get(&(thread->parent)) == NULL){
	...
	time_limit = (1.0 / (1.0 + thread->tic_delay) / thread->delay) * delay_per_hertz * time_unit;
...
ags_thread_loop(void *ptr)
{
  ...
  GList *start_queue, *start_queue_next;
