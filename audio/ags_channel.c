				...
				guint audio_signal_level)
{
  ...
  void ags_channel_recursive_play_init_resolve_audio_recall(AgsAudio *audio,
							    guint audio_channel,
							    AgsGroupId group_id)
  {
    AgsRecall *recall;
							...
							AgsGroupId group_id)
  {
    ...
    /* resolve input */
    ags_channel_recursive_play_init_resolve_channel_recall(input,
							   group_id);
  }
							 ...
							 AgsGroupId group_id)
  {
    ...
    /* resolve output */
    ags_channel_recursive_play_init_resolve_channel_recall(output,
							   group_id);
   ...
   */
  void ags_channel_recursive_play_init_channel_recall(AgsChannel *channel,
						      AgsGroupId group_id)
  {
    AgsRecall *recall;
    AgsRecallID *recall_id;
  ...
  void ags_channel_recursive_play_init_audio_recall(AgsAudio *audio, guint audio_channel,
						    AgsGroupId group_id)
  {
    AgsRecall *recall;
    AgsRecallID *recall_id;
					    ...
					    AgsGroupId group_id)
  {
    ...
    if((AGS_AUDIO_OUTPUT_HAS_RECYCLING & (audio->flags)) != 0){
      ...
      ags_channel_recursive_play_init_audio_recall(audio, output->audio_channel, next_group_id);
  ...
  for(; stage < stage_stop; stage++){
    if(AGS_IS_OUTPUT(channel)){
      if((AGS_AUDIO_OUTPUT_HAS_RECYCLING & (audio->flags)) != 0){	
	ags_channel_recursive_play_init_audio_recall(audio, channel->audio_channel, child_group_id);
...
/*
 * channel - an AgsInput
 * new_toplevel_group_id -
 * old_link - an AgsOutput
 * old_toplevel_group_id -
 */
void
				      ...
				      AgsChannel *old_link, AgsGroupId old_toplevel_group_id)
{
  ...
  GList *devout_play_list, *parent_devout_play_list;
  GList *group_id_list, *child_group_id_list, *invalid_group_id_list;
  ...
  auto GList* ags_channel_group_id_to_child_group_id(AgsChannel *channel,
						     GList *group_id_list);
  auto void ags_channel_recursive_collect_invalid_devout_play_down_input(AgsChannel *output);
  auto void ags_channel_recursive_collect_invalid_devout_play_down(AgsChannel *current);
  auto void ags_channel_collect_invalid_devout_play_up();
  ...
  void ags_audio_reset_group_id(AgsAudio *audio,
    ...
    while(group_id_list != NULL){
      ...
      devout_play = AGS_DEVOUT(devout_play_list->data);
  ...
  GList* ags_channel_group_id_to_child_group_id(AgsChannel *channel, GList *group_id_list){
    AgsRecallID *recall_id;
    ...
    while(list != NULL){
      ...
      recall_id = ags_recall_id_find_parent_group_id(channel->recall_id,
						     group_id);
  ...
  void ags_channel_recursive_collect_invalid_devout_play_down_input(AgsChannel *output){
    AgsAudio *audio;
    AgsChannel *current;
    gboolean check_link;
	...
	if(current->devout_play != NULL){
	  invalid_devout_play_list_of_old = g_list_prepend(invalid_devout_play_list_of_old,
							   current->devout_play);
	}
	/* go down */
	ags_channel_recursive_collect_invalid_group_id_down(current->link);
  ...
  void ags_channel_recursive_collect_invalid_devout_play_down(AgsChannel *current){
    AgsAudio *audio;
    ...
    /* go down */
    ags_channel_recursive_collect_invalid_devout_play_down_input(current);
  }
  void ags_channel_collect_invalid_devout_play_up(){
    AgsAudio *audio;
    AgsChannel *current;
    ...
    while(current != NULL){
      if(current->devout_play != NULL){
	parent_devout_play_list = g_list_prepend(parent_devout_play_list,
						 current->devout_play);
      }
  ...
  devout_play_list = NULL;
  parent_devout_play_list = NULL;

  if(channel->devout_play != NULL){
    invalid_devout_play_list_of_old = g_list_prepend(devout_play_list,
						     channel->devout_play);
  }

  /* go down */
  ags_channel_recursive_collect_invalid_devout_play_down(channel->link);

  /* go up */
  ags_channel_collect_invalid_devout_play_up();

  /* retrieve group_id related lists */
  group_id_list = NULL;
  child_group_id_list = NULL;
  invalid_group_id_list = NULL;

  /* go down */
  group_id_list = ags_channel_devout_play_to_group_id(devout_play_list);

  child_group_id_list = ags_channel_group_id_to_child_group_id(channel,
							       group_id_list);

  /* collect invalid group ids from above */
  ags_channel_collect_invalid_group_id_up(group_id_list);
  ...
  if((AGS_AUDIO_INPUT_HAS_RECYCLING & (audio->flags)) == 0){
    ags_channel_recursive_reset_group_id_down(channel->link,
					      invalid_group_id_list_of_old, child_group_id_list,
					      invalid_group_id_list,
					      0);
  }else{
    GList *new_group_id_list, *new_invalid_group_id_list;

    new_group_id_list = child_group_id_list;
    child_group_id_list = ags_channel_group_id_to_child_group_id(channel, invalid_group_id_list_of_old);
    
    new_invalid_group_id_list = ags_channel_group_id_to_child_group_id(channel, invalid_group_id_list);
    ...
    g_list_free(child_group_id_list);
    g_list_free(new_invalid_group_id_list);
    child_group_id_list = new_group_id_list;
  }

  /* set the new group of channel->link */
  if((AGS_AUDIO_ASYNC & (audio->flags)) != 0){
    AgsChannel *output;

    output = ags_channel_nth(audio->output, channel->audio_channel);
    ags_channel_tillrecycling_set_group_id_up(output);
  }else{
    AgsChannel *output;
  ...
  /* free the lists */
  g_list_free(devout_play_list);
  g_list_free(group_id_list);
  g_list_free(child_group_id_list);
