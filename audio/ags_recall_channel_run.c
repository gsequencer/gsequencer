...
{
  recall_channel_run->devout = NULL;
  ...
  case PROP_DEVOUT:
    {
      ...
      if(recall_channel_run->devout == devout)
	return;

      if(recall_channel_run->devout != NULL)
	g_object_unref(G_OBJECT(recall_channel_run->devout));

      if(devout != NULL)
	g_object_ref(G_OBJECT(devout));

      recall_channel_run->devout = devout;
    }
    break;
				    ...
				    GParamSpec *param_spec)
{
  ...
  case PROP_DEVOUT:
    {
      g_value_set_object(value, recall_channel_run->devout);
    }
    break;
  case PROP_RECALL_AUDIO_RUN:
    {
...
ags_recall_channel_run_finalize(GObject *gobject)
{
  ...
  if(recall_channel_run->devout != NULL)
    g_object_unref(recall_channel_run->devout);
...
ags_recall_channel_run_pack(AgsPackable *packable, GObject *container)
{
  ...
  if(AGS_RECALL(packable)->recall_id != NULL){
    group_id = ags_recall_channel_run_real_get_audio_run_group_id(AGS_RECALL_CHANNEL_RUN(packable));
